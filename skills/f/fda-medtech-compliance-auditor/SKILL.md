...
description: "FDA MedTech Compliance Auditor workflow skill. Use this skill when the user needs Expert AI auditor for Medical Device (SaMD) compliance, IEC 62304, and 21 CFR Part 820. Reviews DHFs, technical files, and software validation and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: testing-security
tags: ["fda-medtech-compliance-auditor", "expert", "auditor", "for", "medical", "device", "samd", "compliance"]
complexity: beginner
risk: caution
...
```text
Use @fda-medtech-compliance-auditor to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @fda-medtech-compliance-auditor against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @fda-medtech-compliance-auditor for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @fda-medtech-compliance-auditor using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/fda-medtech-compliance-auditor`, fails to mention provenance, or does not use any copied source files at all.
