---
name: filesystem-context-v2
description: "Filesystem-Based Context Engineering workflow skill. Use this skill when the user needs Use for file-based context management, dynamic context discovery, and reducing context window bloat. Offload context to files for just-in-time loading and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: ai-agents
tags: ["filesystem-context-v2", "filesystem-context", "use", "for", "file-based", "context", "management", "dynamic"]
complexity: advanced
risk: caution
...
```text
Use @filesystem-context-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @filesystem-context-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @filesystem-context-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @filesystem-context-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
