---
name: flutter-expert-v3
description: "flutter-expert workflow skill. Use this skill when the user needs Master Flutter development with Dart 3, advanced widgets, and multi-platform deployment and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: development
tags: ["flutter-expert-v3", "flutter-expert", "master", "flutter", "development", "dart", "advanced", "widgets"]
complexity: advanced
risk: caution
...
```text
Use @flutter-expert-v3 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @flutter-expert-v3 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @flutter-expert-v3 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @flutter-expert-v3 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-bundle-mobile-developer/skills/flutter-expert`, fails to mention provenance, or does not use any copied source files at all.
