...
version: "0.0.1"
category: frontend
tags: ["fp-ts", "either", "error-handling", "validation", "quick-reference", "fp-either-ref", "quick", "reference"]
complexity: beginner
risk: safe
...
```text
Use @fp-either-ref to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @fp-either-ref against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @fp-either-ref for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @fp-either-ref using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/fp-either-ref`, fails to mention provenance, or does not use any copied source files at all.
