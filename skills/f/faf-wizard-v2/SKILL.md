---
name: faf-wizard-v2
description: "FAF Wizard - One-Click AI Intelligence workflow skill. Use this skill when the user needs Done-for-you .faf generator. One-click AI context for any project - new, legacy, or famous. Auto-detects stack, scores readiness, works everywhere and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: cli-automation
tags: ["faf", "automation", "project-setup", "ai-context", "productivity", "faf-wizard-v2", "faf-wizard", "done-for-you"]
complexity: advanced
risk: caution
...
```text
Use @faf-wizard-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @faf-wizard-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @faf-wizard-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @faf-wizard-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
