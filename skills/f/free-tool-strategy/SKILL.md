...
version: "0.0.1"
category: business
tags: ["free-tool-strategy", "you", "are", "expert", "engineering-as-marketing", "strategy", "your", "goal"]
complexity: advanced
risk: safe
...
```text
Use @free-tool-strategy to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @free-tool-strategy against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @free-tool-strategy for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @free-tool-strategy using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/free-tool-strategy`, fails to mention provenance, or does not use any copied source files at all.
