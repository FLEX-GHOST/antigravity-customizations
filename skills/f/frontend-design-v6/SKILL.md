...
version: "0.0.1"
category: frontend
tags: ["frontend-design-v6", "frontend-design", "you", "are", "frontend", "designer-engineer", "not", "layout"]
...
```text
Use @frontend-design-v6 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @frontend-design-v6 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @frontend-design-v6 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @frontend-design-v6 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-bundle-web-wizard/skills/frontend-design`, fails to mention provenance, or does not use any copied source files at all.
