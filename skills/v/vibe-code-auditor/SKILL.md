...
tags: ["vibe-code-auditor", "audit", "rapidly", "generated", "ai-produced", "for", "structural", "flaws"]
complexity: advanced
risk: caution
...
```text
Use @vibe-code-auditor to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @vibe-code-auditor against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @vibe-code-auditor for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @vibe-code-auditor using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/vibe-code-auditor`, fails to mention provenance, or does not use any copied source files at all.
