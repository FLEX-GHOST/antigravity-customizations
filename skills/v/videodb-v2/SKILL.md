...
| Workflow execution | `scripts/ws_listener.py` | Starts with the smallest copied file that materially changes execution |
| Supporting context | `reference/api-reference.md` | Adds the next most relevant copied source file without loading the entire package |
...
```text
Use @videodb-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @videodb-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @videodb-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @videodb-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/videodb`, fails to mention provenance, or does not use any copied source files at all.
...
| `scripts` | upstream helper scripts that change execution or validation | `scripts/ws_listener.py` |
| `agents` | routing or delegation notes that are genuinely part of the imported package | `agents/n/a` |
| `assets` | supporting assets or schemas copied from the source package | `assets/n/a` |

- [ws_listener.py](scripts/ws_listener.py)
- [api-reference.md](reference/api-reference.md)
- [capture-reference.md](reference/capture-reference.md)
- [capture.md](reference/capture.md)
- [editor.md](reference/editor.md)
- [generative.md](reference/generative.md)
