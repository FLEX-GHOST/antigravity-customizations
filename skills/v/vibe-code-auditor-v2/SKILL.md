---
name: vibe-code-auditor-v2
description: "Vibe Code Auditor workflow skill. Use this skill when the user needs Audit rapidly generated or AI-produced code for structural flaws, fragility, and production risks and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: development
tags: ["vibe-code-auditor-v2", "vibe-code-auditor", "audit", "rapidly", "generated", "ai-produced", "for", "structural"]
complexity: advanced
risk: caution
...
Imported source sections that did not map cleanly to the public headings are still preserved below or in the support files. Notable imported sections: Identity, Purpose, Pre-Audit Checklist, Audit Dimensions, Output Format, Task-Specific Inputs.
...
```text
Use @vibe-code-auditor-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @vibe-code-auditor-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @vibe-code-auditor-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @vibe-code-auditor-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/vibe-code-auditor`, fails to mention provenance, or does not use any copied source files at all.
