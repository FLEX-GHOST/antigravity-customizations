---
name: viboscope-v2
description: "Viboscope workflow skill. Use this skill when the user needs Psychological compatibility matching \u2014 find cofounders, collaborators, and friends through validated psychometrics and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: ai-agents
tags: ["matching", "psychology", "compatibility", "networking", "collaboration", "viboscope-v2", "viboscope", "psychological"]
complexity: beginner
risk: safe
...
- Use when the request clearly matches the imported source intent: Psychological compatibility matching — find cofounders, collaborators, and friends through validated psychometrics.
...
```text
Use @viboscope-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @viboscope-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @viboscope-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @viboscope-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/viboscope`, fails to mention provenance, or does not use any copied source files at all.
