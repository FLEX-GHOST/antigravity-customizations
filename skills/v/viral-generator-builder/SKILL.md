...
description: "Viral Generator Builder workflow skill. Use this skill when the user needs Expert in building shareable generator tools that go viral - name and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: tools
tags: ["viral-generator-builder", "expert", "building", "shareable", "generator", "tools", "viral", "name"]
complexity: advanced
risk: safe
...
```text
Use @viral-generator-builder to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @viral-generator-builder against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @viral-generator-builder for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @viral-generator-builder using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/viral-generator-builder`, fails to mention provenance, or does not use any copied source files at all.
