---
name: vscode-extension-guide-en-v2
description: "VS Code Extension Guide (English) workflow skill. Use this skill when the user needs Guide for VS Code extension development from scaffolding to Marketplace publication and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: tools
tags: ["vscode", "extension", "ide", "typescript", "marketplace", "vscode-extension-guide-en-v2", "vscode-extension-guide-en", "guide"]
complexity: beginner
risk: safe
...
```text
Use @vscode-extension-guide-en-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @vscode-extension-guide-en-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @vscode-extension-guide-en-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @vscode-extension-guide-en-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/vscode-extension-guide-en`, fails to mention provenance, or does not use any copied source files at all.
