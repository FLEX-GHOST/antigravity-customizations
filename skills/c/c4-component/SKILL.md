...
version: "0.0.1"
category: frontend
tags: ["c4-component", "expert", "component-level", "documentation", "specialist", "synthesizes", "code-level", "architecture"]
...
```text
Use @c4-component to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @c4-component against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @c4-component for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @c4-component using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/c4-component`, fails to mention provenance, or does not use any copied source files at all.
