...
tags: ["create-issue-gate-v2", "create-issue-gate", "starting", "new", "implementation", "task", "and", "issue"]
complexity: beginner
risk: safe
...
```text
Use @create-issue-gate-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @create-issue-gate-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @create-issue-gate-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @create-issue-gate-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/create-issue-gate`, fails to mention provenance, or does not use any copied source files at all.
...
#### Imported: Dependencies/Blockers
