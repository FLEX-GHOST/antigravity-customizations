---
name: code-review-excellence-v2
description: "Code Review Excellence workflow skill. Use this skill when the user needs Transform code reviews from gatekeeping to knowledge sharing through constructive feedback, systematic analysis, and collaborative improvement and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: development
tags: ["code-review-excellence-v2", "code-review-excellence", "transform", "reviews", "gatekeeping", "knowledge", "sharing", "constructive"]
complexity: beginner
risk: safe
...
1. Read context, requirements, and test signals first.
2. Review for correctness, security, performance, and maintainability.
3. Provide actionable feedback with severity and rationale.
4. Ask clarifying questions when intent is unclear.
5. If detailed checklists are required, open resources/implementation-playbook.md.
...
```text
Use @code-review-excellence-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @code-review-excellence-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @code-review-excellence-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @code-review-excellence-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/code-review-excellence`, fails to mention provenance, or does not use any copied source files at all.
