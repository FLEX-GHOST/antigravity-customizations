...
version: "0.0.1"
category: ai-agents
tags: ["context-management-context-restore", "working", "context", "management", "restore", "ai-agents"]
...
```text
Use @context-management-context-restore to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @context-management-context-restore against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @context-management-context-restore for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @context-management-context-restore using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/context-management-context-restore`, fails to mention provenance, or does not use any copied source files at all.
