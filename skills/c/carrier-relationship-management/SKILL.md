...
version: "0.0.1"
category: frontend
tags: ["carrier-relationship-management", "codified", "expertise", "for", "managing", "carrier", "portfolios", "negotiating"]
complexity: advanced
risk: caution
...
```text
Use @carrier-relationship-management to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @carrier-relationship-management against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @carrier-relationship-management for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @carrier-relationship-management using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/carrier-relationship-management`, fails to mention provenance, or does not use any copied source files at all.
