...
| Workflow execution | `references/coding-principles.md` | Starts with the smallest copied file that materially changes execution |
| Supporting context | `references/notebook-spec.md` | Adds the next most relevant copied source file without loading the entire package |
...
```text
Use @codenavi to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @codenavi against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @codenavi for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @codenavi using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `packages/skills-catalog/skills/(development)/codenavi`, fails to mention provenance, or does not use any copied source files at all.
...
| --- | --- | --- |
| `references` | copied reference notes, guides, or background material from upstream | `references/coding-principles.md` |
...
- [notebook-spec.md](references/notebook-spec.md)
- [coding-principles.md](references/coding-principles.md)
- [notebook-spec.md](references/notebook-spec.md)
