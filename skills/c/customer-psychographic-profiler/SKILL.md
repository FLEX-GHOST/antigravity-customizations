...
version: "0.0.1"
category: tools
tags: ["customer-psychographic-profiler", "one", "sentence", "what", "does", "and", "invoke", "tools"]
complexity: advanced
risk: safe
...
```text
Use @customer-psychographic-profiler to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @customer-psychographic-profiler against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @customer-psychographic-profiler for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @customer-psychographic-profiler using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/customer-psychographic-profiler`, fails to mention provenance, or does not use any copied source files at all.
