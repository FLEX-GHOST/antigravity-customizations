...
tags: ["comprehensive-review-pr-enhance", "testing-security", "resources"]
complexity: beginner
risk: caution
...
```text
Use @comprehensive-review-pr-enhance to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @comprehensive-review-pr-enhance against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @comprehensive-review-pr-enhance for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @comprehensive-review-pr-enhance using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/comprehensive-review-pr-enhance`, fails to mention provenance, or does not use any copied source files at all.
