...
description: "Context Restoration: Advanced Semantic Memory Rehydration workflow skill. Use this skill when the user needs working with code refactoring context restore and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: development
tags: ["code-refactoring-context-restore", "working", "refactoring", "context", "restore", "development"]
...
```text
Use @code-refactoring-context-restore to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @code-refactoring-context-restore against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @code-refactoring-context-restore for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @code-refactoring-context-restore using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/code-refactoring-context-restore`, fails to mention provenance, or does not use any copied source files at all.
