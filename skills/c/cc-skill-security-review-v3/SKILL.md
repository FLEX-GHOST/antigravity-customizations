---
name: cc-skill-security-review-v3
description: "Security Review Skill workflow skill. Use this skill when the user needs This skill ensures all code follows security best practices and identifies potential vulnerabilities. Use when implementing authentication or authorization, handling user input or file uploads, or creating new API endpoints and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: backend
tags: ["cc-skill-security-review-v3", "cc-skill-security-review", "ensures", "all", "follows", "security", "best", "practices"]
complexity: advanced
risk: caution
...
```text
Use @cc-skill-security-review-v3 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @cc-skill-security-review-v3 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @cc-skill-security-review-v3 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @cc-skill-security-review-v3 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-bundle-security-developer/skills/cc-skill-security-review`, fails to mention provenance, or does not use any copied source files at all.
