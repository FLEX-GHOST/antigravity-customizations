---
name: ui-a11y-v2
description: "UI Accessibility Audit workflow skill. Use this skill when the user needs Audit a StyleSeed-based component or page for WCAG 2.2 AA issues and apply practical accessibility fixes where the code makes them safe and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: design
tags: ["ui", "accessibility", "wcag", "audit", "styleseed", "ui-a11y-v2", "ui-a11y", "styleseed-based"]
complexity: beginner
risk: safe
...
Imported source sections that did not map cleanly to the public headings are still preserved below or in the support files. Notable imported sections: Audit Areas, Output, Limitations.
...
- Use when the request clearly matches the imported source intent: Audit a StyleSeed-based component or page for WCAG 2.2 AA issues and apply practical accessibility fixes where the code makes them safe.
...
```text
Use @ui-a11y-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @ui-a11y-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @ui-a11y-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @ui-a11y-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/ui-a11y`, fails to mention provenance, or does not use any copied source files at all.
