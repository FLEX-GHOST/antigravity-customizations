...
version: "0.0.1"
category: design
tags: ["ui", "accessibility", "wcag", "audit", "styleseed", "ui-a11y", "styleseed-based", "component"]
complexity: beginner
risk: safe
...
```text
Use @ui-a11y to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @ui-a11y against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @ui-a11y for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @ui-a11y using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/ui-a11y`, fails to mention provenance, or does not use any copied source files at all.
