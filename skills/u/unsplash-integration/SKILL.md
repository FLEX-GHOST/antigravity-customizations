...
tags: ["unsplash-integration", "integration", "for", "searching", "and", "fetching", "high-quality", "free-to-use"]
complexity: beginner
risk: safe
...
```text
Use @unsplash-integration to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @unsplash-integration against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @unsplash-integration for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @unsplash-integration using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/unsplash-integration`, fails to mention provenance, or does not use any copied source files at all.
