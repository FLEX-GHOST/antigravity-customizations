...
version: "0.0.1"
category: design
tags: ["ui-ux-pro-max-v2", "ui-ux-pro-max", "comprehensive", "design", "guide", "for", "web", "and"]
complexity: advanced
risk: safe
...
```text
Use @ui-ux-pro-max-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @ui-ux-pro-max-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @ui-ux-pro-max-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @ui-ux-pro-max-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/ui-ux-pro-max`, fails to mention provenance, or does not use any copied source files at all.
