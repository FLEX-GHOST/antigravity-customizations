---
name: unreal-engine-cpp-pro-v2
description: "Unreal Engine C++ Pro workflow skill. Use this skill when the user needs Expert guide for Unreal Engine 5.x C++ development, covering UObject hygiene, performance patterns, and best practices and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: development
tags: ["unreal-engine-cpp-pro-v2", "unreal-engine-cpp-pro", "expert", "guide", "for", "unreal", "engine", "c"]
...
| Workflow execution | `examples/ExampleActor.cpp` | Starts with the smallest copied file that materially changes execution |
| Supporting context | `examples/ExampleActor.h` | Adds the next most relevant copied source file without loading the entire package |
...
```text
Use @unreal-engine-cpp-pro-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @unreal-engine-cpp-pro-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @unreal-engine-cpp-pro-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @unreal-engine-cpp-pro-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/unreal-engine-cpp-pro`, fails to mention provenance, or does not use any copied source files at all.
...
| `examples` | worked examples or reusable prompts copied from upstream | `examples/ExampleActor.cpp` |
| `scripts` | upstream helper scripts that change execution or validation | `scripts/n/a` |
| `agents` | routing or delegation notes that are genuinely part of the imported package | `agents/n/a` |
| `assets` | supporting assets or schemas copied from the source package | `assets/n/a` |

- [ExampleActor.cpp](examples/ExampleActor.cpp)
- [ExampleActor.h](examples/ExampleActor.h)
- [ExampleActor.cpp](examples/ExampleActor.cpp)
- [ExampleActor.h](examples/ExampleActor.h)
