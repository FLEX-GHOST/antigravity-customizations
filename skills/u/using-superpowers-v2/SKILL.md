---
name: using-superpowers-v2
description: "Using Skills workflow skill. Use this skill when the user needs starting any conversation - establishes how to find and use skills, requiring Skill tool invocation before ANY response including clarifying questions and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: frontend
tags: ["using-superpowers-v2", "using-superpowers", "starting", "any", "conversation", "establishes", "how", "find"]
...
```text
Use @using-superpowers-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @using-superpowers-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @using-superpowers-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @using-superpowers-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/using-superpowers`, fails to mention provenance, or does not use any copied source files at all.
