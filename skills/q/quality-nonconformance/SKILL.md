...
tags: ["quality-nonconformance", "codified", "expertise", "for", "quality", "control", "non-conformance", "investigation"]
complexity: advanced
risk: caution
...
```text
Review @quality-nonconformance against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/quality-nonconformance`, fails to mention provenance, or does not use any copied source files at all.
