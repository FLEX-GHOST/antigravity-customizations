---
name: receiving-code-review-v3
description: "Code Review Reception workflow skill. Use this skill when the user needs Code review requires technical evaluation, not emotional performance and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: development
tags: ["receiving-code-review-v3", "receiving-code-review", "review", "requires", "technical", "evaluation", "not", "emotional"]
...
```text
Use @receiving-code-review-v3 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @receiving-code-review-v3 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @receiving-code-review-v3 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @receiving-code-review-v3 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-bundle-oss-maintainer/skills/receiving-code-review`, fails to mention provenance, or does not use any copied source files at all.
