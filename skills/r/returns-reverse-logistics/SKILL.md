...
version: "0.0.1"
category: devops
tags: ["returns-reverse-logistics", "codified", "expertise", "for", "returns", "authorisation", "receipt", "and"]
complexity: advanced
risk: caution
...
```text
Use @returns-reverse-logistics to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @returns-reverse-logistics against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @returns-reverse-logistics for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @returns-reverse-logistics using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/returns-reverse-logistics`, fails to mention provenance, or does not use any copied source files at all.
