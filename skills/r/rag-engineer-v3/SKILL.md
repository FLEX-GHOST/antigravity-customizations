---
name: rag-engineer-v3
description: "RAG Engineer workflow skill. Use this skill when the user needs Expert in building Retrieval-Augmented Generation systems. Masters and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: ai-agents
tags: ["rag-engineer-v3", "rag-engineer", "expert", "building", "retrieval-augmented", "generation", "systems", "masters"]
complexity: advanced
risk: safe
...
```text
Use @rag-engineer-v3 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @rag-engineer-v3 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @rag-engineer-v3 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @rag-engineer-v3 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-bundle-agent-architect/skills/rag-engineer`, fails to mention provenance, or does not use any copied source files at all.
