...
version: "0.0.1"
category: devops
tags: ["risk-metrics-calculation", "calculate", "portfolio", "risk", "metrics", "including", "var", "cvar"]
complexity: beginner
risk: safe
...
```text
Use @risk-metrics-calculation to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @risk-metrics-calculation against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @risk-metrics-calculation for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @risk-metrics-calculation using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/risk-metrics-calculation`, fails to mention provenance, or does not use any copied source files at all.
