---
name: backend-dev-guidelines-v3
description: "Backend Development Guidelines workflow skill. Use this skill when the user needs You are a senior backend engineer operating production-grade services under strict architectural and reliability constraints. Use when routes, controllers, services, repositories, express middleware, or prisma database access and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: backend
tags: ["backend-dev-guidelines-v3", "backend-dev-guidelines", "you", "are", "senior", "backend", "engineer", "operating"]
complexity: advanced
risk: caution
...
| Workflow execution | `resources/architecture-overview.md` | Starts with the smallest copied file that materially changes execution |
| Supporting context | `resources/async-and-errors.md` | Adds the next most relevant copied source file without loading the entire package |
...
```text
Use @backend-dev-guidelines-v3 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @backend-dev-guidelines-v3 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @backend-dev-guidelines-v3 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @backend-dev-guidelines-v3 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-bundle-full-stack-developer/skills/backend-dev-guidelines`, fails to mention provenance, or does not use any copied source files at all.
