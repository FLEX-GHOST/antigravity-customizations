...
version: "0.0.1"
category: product
tags: ["buywhere", "ecommerce", "shopping", "mcp", "api", "product-catalog", "buywhere-product-catalog", "use"]
...
```text
Use @buywhere-product-catalog to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @buywhere-product-catalog against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @buywhere-product-catalog for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @buywhere-product-catalog using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/buywhere-product-catalog`, fails to mention provenance, or does not use any copied source files at all.
