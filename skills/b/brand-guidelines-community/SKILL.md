...
version: "0.0.1"
category: tools
tags: ["brand-guidelines-community", "access", "anthropic", "official", "brand", "identity", "and", "style"]
complexity: beginner
risk: safe
...
```text
Use @brand-guidelines-community to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @brand-guidelines-community against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @brand-guidelines-community for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @brand-guidelines-community using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/brand-guidelines-community`, fails to mention provenance, or does not use any copied source files at all.
