...
tags: ["bash-linux-v3", "bash-linux", "bash", "linux", "terminal", "patterns", "critical", "commands"]
...
```text
Use @bash-linux-v3 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @bash-linux-v3 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @bash-linux-v3 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @bash-linux-v3 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-bundle-devops-cloud/skills/bash-linux`, fails to mention provenance, or does not use any copied source files at all.
