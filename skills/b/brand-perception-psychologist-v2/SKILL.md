---
name: brand-perception-psychologist-v2
description: "brand-perception-psychologist workflow skill. Use this skill when the user needs One sentence - what this skill does and when to invoke it and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: ai-agents
tags: ["brand-perception-psychologist-v2", "brand-perception-psychologist", "one", "sentence", "what", "does", "and", "invoke"]
...
```text
Use @brand-perception-psychologist-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @brand-perception-psychologist-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @brand-perception-psychologist-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @brand-perception-psychologist-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/brand-perception-psychologist`, fails to mention provenance, or does not use any copied source files at all.
