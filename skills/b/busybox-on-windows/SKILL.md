...
version: "0.0.1"
category: frontend
tags: ["busybox-on-windows", "how", "use", "win32", "build", "busybox", "run", "many"]
complexity: beginner
risk: safe
...
```text
Use @busybox-on-windows to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @busybox-on-windows against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @busybox-on-windows for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @busybox-on-windows using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/busybox-on-windows`, fails to mention provenance, or does not use any copied source files at all.
