...
version: "0.0.1"
category: business
tags: ["billing-automation", "master", "automated", "billing", "systems", "including", "recurring", "invoice"]
complexity: beginner
risk: caution
...
```text
Use @billing-automation to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @billing-automation against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @billing-automation for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @billing-automation using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/billing-automation`, fails to mention provenance, or does not use any copied source files at all.
