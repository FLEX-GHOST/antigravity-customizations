---
name: busybox-on-windows-v2
description: "busybox-on-windows workflow skill. Use this skill when the user needs How to use a Win32 build of BusyBox to run many of the standard UNIX command line tools on Windows and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: frontend
tags: ["busybox-on-windows-v2", "busybox-on-windows", "how", "use", "win32", "build", "busybox", "run"]
complexity: beginner
risk: safe
...
- Use when the request clearly matches the imported source intent: How to use a Win32 build of BusyBox to run many of the standard UNIX command line tools on Windows.
...
```text
Use @busybox-on-windows-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @busybox-on-windows-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @busybox-on-windows-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @busybox-on-windows-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/busybox-on-windows`, fails to mention provenance, or does not use any copied source files at all.
