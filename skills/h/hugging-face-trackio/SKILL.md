...
description: "Trackio - Experiment Tracking for ML Training workflow skill. Use this skill when the user needs Track ML experiments with Trackio using Python logging, alerts, and CLI metric retrieval and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: cli-automation
tags: ["hugging-face-trackio", "track", "experiments", "trackio", "using", "python", "logging", "alerts"]
...
| Workflow execution | `references/alerts.md` | Starts with the smallest copied file that materially changes execution |
| Supporting context | `references/logging_metrics.md` | Adds the next most relevant copied source file without loading the entire package |
...
```text
Use @hugging-face-trackio to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @hugging-face-trackio against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @hugging-face-trackio for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @hugging-face-trackio using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/hugging-face-trackio`, fails to mention provenance, or does not use any copied source files at all.
...
| --- | --- | --- |
| `references` | copied reference notes, guides, or background material from upstream | `references/alerts.md` |
...
- [logging_metrics.md](references/logging_metrics.md)
- [retrieving_metrics.md](references/retrieving_metrics.md)
