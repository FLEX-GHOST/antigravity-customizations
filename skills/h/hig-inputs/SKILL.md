...
version: "0.0.1"
category: design
tags: ["hig-inputs", "check", "for", "claude", "apple-design-context", "asking", "questions", "use"]
complexity: advanced
risk: safe
...
```text
Use @hig-inputs to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @hig-inputs against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @hig-inputs for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @hig-inputs using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/hig-inputs`, fails to mention provenance, or does not use any copied source files at all.
