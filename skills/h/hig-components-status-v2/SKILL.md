---
name: hig-components-status-v2
description: "Apple HIG: Status Components workflow skill. Use this skill when the user needs Apple HIG guidance for status and progress UI components including progress indicators, status bars, and activity rings and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: frontend
tags: ["hig-components-status-v2", "hig-components-status", "apple", "hig", "guidance", "for", "status", "and"]
complexity: beginner
risk: safe
...
# Apple HIG: Status Components Check for .claude/apple-design-context.md before asking questions. Use existing context and only ask for information not already covered.
...
- Use when the request clearly matches the imported source intent: Apple HIG guidance for status and progress UI components including progress indicators, status bars, and activity rings.
...
| Workflow execution | `references/activity-rings.md` | Starts with the smallest copied file that materially changes execution |
| Supporting context | `references/progress-indicators.md` | Adds the next most relevant copied source file without loading the entire package |
...
```text
Use @hig-components-status-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @hig-components-status-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @hig-components-status-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @hig-components-status-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/hig-components-status`, fails to mention provenance, or does not use any copied source files at all.
...
| --- | --- | --- |
| `references` | copied reference notes, guides, or background material from upstream | `references/activity-rings.md` |
| `examples` | worked examples or reusable prompts copied from upstream | `examples/n/a` |
| `scripts` | upstream helper scripts that change execution or validation | `scripts/n/a` |
| `agents` | routing or delegation notes that are genuinely part of the imported package | `agents/n/a` |
| `assets` | supporting assets or schemas copied from the source package | `assets/n/a` |

- [activity-rings.md](references/activity-rings.md)
- [progress-indicators.md](references/progress-indicators.md)
- [status-bars.md](references/status-bars.md)
- [activity-rings.md](references/activity-rings.md)
- [progress-indicators.md](references/progress-indicators.md)
- [status-bars.md](references/status-bars.md)
