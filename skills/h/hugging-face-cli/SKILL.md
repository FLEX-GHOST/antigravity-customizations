...
tags: ["hugging-face-cli", "use", "the", "hugging", "face", "hub", "cli", "download"]
complexity: advanced
risk: caution
...
```text
Use @hugging-face-cli to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @hugging-face-cli against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @hugging-face-cli for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @hugging-face-cli using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/hugging-face-cli`, fails to mention provenance, or does not use any copied source files at all.
