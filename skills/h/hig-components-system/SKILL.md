...
version: "0.0.1"
category: frontend
tags: ["hig-components-system", "apple", "hig", "guidance", "for", "system", "experience", "components"]
complexity: advanced
risk: caution
...
```text
Use @hig-components-system to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @hig-components-system against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @hig-components-system for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @hig-components-system using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/hig-components-system`, fails to mention provenance, or does not use any copied source files at all.
