...
tags: ["hubspot-integration-v4", "hubspot-integration", "expert", "patterns", "for", "hubspot", "crm", "integration"]
complexity: advanced
risk: caution
...
```text
Use @hubspot-integration-v4 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @hubspot-integration-v4 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @hubspot-integration-v4 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @hubspot-integration-v4 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-bundle-integration-apis/skills/hubspot-integration`, fails to mention provenance, or does not use any copied source files at all.
