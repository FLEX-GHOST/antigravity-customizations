...
version: "0.0.1"
category: frontend
tags: ["hig-components-search", "apple", "hig", "guidance", "for", "navigation-related", "components", "including"]
complexity: beginner
risk: safe
...
```text
Use @hig-components-search to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @hig-components-search against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @hig-components-search for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @hig-components-search using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/hig-components-search`, fails to mention provenance, or does not use any copied source files at all.
