---
name: hubspot-integration-v3
description: "HubSpot Integration workflow skill. Use this skill when the user needs Expert patterns for HubSpot CRM integration including OAuth and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: testing-security
tags: ["hubspot-integration-v3", "hubspot-integration", "expert", "patterns", "for", "hubspot", "crm", "integration"]
complexity: advanced
risk: caution
...
```text
Use @hubspot-integration-v3 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @hubspot-integration-v3 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @hubspot-integration-v3 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @hubspot-integration-v3 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-bundle-commerce-payments/skills/hubspot-integration`, fails to mention provenance, or does not use any copied source files at all.
