---
name: hig-project-context-v2
description: "Apple HIG: Project Context workflow skill. Use this skill when the user needs Create or update a shared Apple design context document that other HIG skills use to tailor guidance and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: design
tags: ["hig-project-context-v2", "hig-project-context", "create", "update", "shared", "apple", "design", "context"]
...
# Apple HIG: Project Context Create and maintain .claude/apple-design-context.md so other HIG skills can skip redundant questions. Check for .claude/apple-design-context.md before asking questions. Use existing context and only ask for information not already covered.

Imported source sections that did not map cleanly to the public headings are still preserved below or in the support files. Notable imported sections: Gathering Context, Context Document Template, Product, Platforms, Technology, Design System.
...
- Use when the request clearly matches the imported source intent: Create or update a shared Apple design context document that other HIG skills use to tailor guidance.
...
```text
Use @hig-project-context-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @hig-project-context-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @hig-project-context-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @hig-project-context-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/hig-project-context`, fails to mention provenance, or does not use any copied source files at all.
...
#### Imported: Context Document Template
