...
version: "0.0.1"
category: frontend
tags: ["andruia-niche-intelligence", "estratega", "inteligencia", "dominio", "andru", "analiza", "nicho", "espec"]
complexity: beginner
risk: safe
...
- Use this skill once the project's niche or industry has been identified.
- It is essential for injecting domain-specific intelligence, regulatory requirements, and industry-standard UX patterns into the project.
- # 🧠 Andru.ia Niche Intelligence (Dominio Experto)
...
```text
Use @20-andruia-niche-intelligence to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @20-andruia-niche-intelligence against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @20-andruia-niche-intelligence for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @20-andruia-niche-intelligence using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/20-andruia-niche-intelligence`, fails to mention provenance, or does not use any copied source files at all.
