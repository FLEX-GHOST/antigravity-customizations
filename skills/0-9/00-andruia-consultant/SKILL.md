...
version: "0.0.1"
category: frontend
tags: ["andruia-consultant", "arquitecto", "soluciones", "principal", "consultor", "tecnol", "gico", "andru"]
complexity: beginner
risk: safe
...
```text
Use @00-andruia-consultant to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @00-andruia-consultant against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @00-andruia-consultant for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @00-andruia-consultant using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/00-andruia-consultant`, fails to mention provenance, or does not use any copied source files at all.
