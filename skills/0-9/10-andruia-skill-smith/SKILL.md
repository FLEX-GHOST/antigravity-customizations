...
version: "0.0.1"
category: frontend
tags: ["andruia-skill-smith", "ingeniero", "sistemas", "andru", "dise", "redacta", "despliega", "nuevas"]
complexity: beginner
risk: safe
...
```text
Use @10-andruia-skill-smith to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @10-andruia-skill-smith against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @10-andruia-skill-smith for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @10-andruia-skill-smith using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/10-andruia-skill-smith`, fails to mention provenance, or does not use any copied source files at all.
