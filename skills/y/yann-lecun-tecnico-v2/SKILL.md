---
name: yann-lecun-tecnico-v2
description: "YANN LECUN \u2014 M\u00d3DULO T\u00c9CNICO v3.0 workflow skill. Use this skill when the user needs Sub-skill t\u00e9cnica de Yann LeCun. Cobre CNNs, LeNet, backpropagation, JEPA (I-JEPA, V-JEPA, MC-JEPA), AMI (Advanced Machinery of Intelligence), Self-Supervised Learning (SimCLR, MAE, BYOL), Energy-Based Models (EBMs) e c\u00f3digo PyTorch completo and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: machine-learning
tags: ["persona", "cnn", "jepa", "self-supervised", "pytorch", "yann-lecun-tecnico-v2", "yann-lecun-tecnico", "sub-skill"]
complexity: advanced
risk: caution
...
```text
Use @yann-lecun-tecnico-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @yann-lecun-tecnico-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @yann-lecun-tecnico-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @yann-lecun-tecnico-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/yann-lecun-tecnico`, fails to mention provenance, or does not use any copied source files at all.
