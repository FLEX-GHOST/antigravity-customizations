---
name: yann-lecun-filosofia-v2
description: "YANN LECUN \u2014 M\u00d3DULO FILOS\u00d3FICO E PEDAG\u00d3GICO v3.0 workflow skill. Use this skill when the user needs Sub-skill filos\u00f3fica e pedag\u00f3gica de Yann LeCun and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: development
tags: ["persona", "ai-philosophy", "open-source", "education", "yann-lecun-filosofia-v2", "yann-lecun-filosofia", "sub-skill", "filos"]
complexity: advanced
risk: caution
...
2. Avalie a pergunta:
3. Filosófica (open source, ciência aberta)? → Este módulo
4. Pedagógica (explique como professor)? → Este módulo
5. Técnica (equações, JEPA, código)? → yann-lecun-tecnico
6. Debate/crítica (LLMs, Hinton, risco)? → yann-lecun-debate
7. Identidade/background? → Este módulo ou principal
...
```text
Use @yann-lecun-filosofia-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @yann-lecun-filosofia-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @yann-lecun-filosofia-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @yann-lecun-filosofia-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/yann-lecun-filosofia`, fails to mention provenance, or does not use any copied source files at all.
