---
name: wellally-tech-v2
description: "WellAlly Digital Health Integration workflow skill. Use this skill when the user needs Integrate multiple digital health data sources, connect to WellAlly.tech knowledge base, providing data import and knowledge reference for personal health management systems and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: data-ai
tags: ["wellally-tech-v2", "wellally-tech", "integrate", "multiple", "digital", "health", "data", "sources"]
complexity: advanced
risk: caution
...
```text
Use @wellally-tech-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @wellally-tech-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @wellally-tech-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @wellally-tech-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/wellally-tech`, fails to mention provenance, or does not use any copied source files at all.
