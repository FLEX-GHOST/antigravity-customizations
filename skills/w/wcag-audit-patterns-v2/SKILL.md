---
name: wcag-audit-patterns-v2
description: "WCAG Audit Patterns workflow skill. Use this skill when the user needs Comprehensive guide to auditing web content against WCAG 2.2 guidelines with actionable remediation strategies and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: testing-security
tags: ["wcag-audit-patterns-v2", "wcag-audit-patterns", "comprehensive", "guide", "auditing", "web", "content", "wcag"]
complexity: beginner
risk: safe
...
```text
Use @wcag-audit-patterns-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @wcag-audit-patterns-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @wcag-audit-patterns-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @wcag-audit-patterns-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/wcag-audit-patterns`, fails to mention provenance, or does not use any copied source files at all.
