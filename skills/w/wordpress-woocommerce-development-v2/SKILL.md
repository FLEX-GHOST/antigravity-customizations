---
name: wordpress-woocommerce-development-v2
description: "WordPress WooCommerce Development Workflow workflow skill. Use this skill when the user needs WooCommerce store development workflow covering store setup, payment integration, shipping configuration, customization, and WordPress 7.0 features: AI connectors, DataViews, and collaboration tools and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: development
tags: ["wordpress-woocommerce-development-v2", "wordpress-woocommerce-development", "woocommerce", "store", "development", "covering", "setup", "payment"]
complexity: advanced
risk: safe
...
Imported source sections that did not map cleanly to the public headings are still preserved below or in the support files. Notable imported sections: WordPress 7.0 + WooCommerce Features, WooCommerce + WordPress 7.0 AI Use Cases, Quality Gates, Limitations.
...
2. wordpress-penetration-testing - WordPress patterns
3. Install WooCommerce
4. Run setup wizard
5. Configure store settings
6. Set up tax rules
7. Configure currency
...
#### Imported: WordPress 7.0 + WooCommerce Features
...
```text
Use @wordpress-woocommerce-development-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @wordpress-woocommerce-development-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @wordpress-woocommerce-development-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @wordpress-woocommerce-development-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/wordpress-woocommerce-development`, fails to mention provenance, or does not use any copied source files at all.
...
#### Imported: WooCommerce + WordPress 7.0 AI Use Cases
