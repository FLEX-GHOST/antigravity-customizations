---
name: windows-shell-reliability-v2
description: "Windows Shell Reliability Patterns workflow skill. Use this skill when the user needs Reliable command execution on Windows: paths, encoding, and common binary pitfalls and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: cli-automation
tags: ["windows-shell-reliability-v2", "windows-shell-reliability", "reliable", "command", "execution", "windows", "paths", "encoding"]
...
# Windows Shell Reliability Patterns > Best practices for running commands on Windows via PowerShell and CMD.

Imported source sections that did not map cleanly to the public headings are still preserved below or in the support files. Notable imported sections: 1. Encoding & Redirection, 2. Handling Paths & Spaces, 3. Common Binary & Cmdlet Pitfalls, 4. Dotnet CLI Reliability, 5. Environment Variables, 6. Long Paths.
...
- Use when the request clearly matches the imported source intent: Reliable command execution on Windows: paths, encoding, and common binary pitfalls.
...
#### Imported: 1. Encoding & Redirection
...
```text
Use @windows-shell-reliability-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @windows-shell-reliability-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @windows-shell-reliability-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @windows-shell-reliability-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/windows-shell-reliability`, fails to mention provenance, or does not use any copied source files at all.
...
#### Imported: 7. Troubleshooting Shell Errors
...
#### Imported: 2. Handling Paths & Spaces
...
#### Imported: 3. Common Binary & Cmdlet Pitfalls
...
#### Imported: 4. Dotnet CLI Reliability
