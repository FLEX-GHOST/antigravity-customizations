...
tags: ["windows-shell-reliability", "reliable", "command", "execution", "windows", "paths", "encoding", "and"]
...
```text
Use @windows-shell-reliability to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @windows-shell-reliability against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @windows-shell-reliability for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @windows-shell-reliability using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/windows-shell-reliability`, fails to mention provenance, or does not use any copied source files at all.
