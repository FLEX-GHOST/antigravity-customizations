...
tags: ["wcag-audit-patterns", "comprehensive", "guide", "auditing", "web", "content", "wcag", "guidelines"]
complexity: beginner
risk: safe
...
```text
Use @wcag-audit-patterns to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @wcag-audit-patterns against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @wcag-audit-patterns for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @wcag-audit-patterns using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/wcag-audit-patterns`, fails to mention provenance, or does not use any copied source files at all.
