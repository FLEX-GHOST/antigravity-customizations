---
name: minimalist-ui-v2
description: "Protocol: Premium Utilitarian Minimalism UI Architect workflow skill. Use this skill when the user needs creating clean editorial interfaces with warm monochrome palettes, crisp borders, restrained motion, and flat bento layouts and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: frontend
tags: ["frontend", "design", "minimalism", "ui", "minimalist-ui-v2", "minimalist-ui", "creating", "clean"]
complexity: advanced
risk: safe
...
```text
Use @minimalist-ui-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @minimalist-ui-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @minimalist-ui-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @minimalist-ui-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/minimalist-ui`, fails to mention provenance, or does not use any copied source files at all.
