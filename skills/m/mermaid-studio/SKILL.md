...
# Mermaid Studio Expert-level Mermaid diagram creation, validation, and multi-format rendering. Creates diagrams from descriptions or code analysis, validates syntax, and renders to SVG, PNG, or ASCII with professional theming.

Imported source sections that did not map cleanly to the public headings are still preserved below or in the support files. Notable imported sections: Modes of Operation, C4 Diagrams — Mandatory Styling Guide, Output Conventions.
...
| Workflow execution | `references/aws-architecture.md` | Starts with the smallest copied file that materially changes execution |
| Supporting context | `references/c4-architecture.md` | Adds the next most relevant copied source file without loading the entire package |
...
5. User describes... - Diagram Type - Syntax keyword
6. Process, algorithm, decision tree, workflow - Flowchart - flowchart TD/LR
7. API calls, message passing, request/response - Sequence - sequenceDiagram

### Imported Workflow Notes

#### Imported: Step 1: Understand the Request
...
#### Imported: Step 2: Create the Diagram
...
```text
Use @mermaid-studio to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @mermaid-studio against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @mermaid-studio for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @mermaid-studio using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
#### Imported: Golden Rules — Elegant Diagrams by Default
...
**Symptoms:** The result ignores the upstream workflow in `packages/skills-catalog/skills/(tooling)/mermaid-studio`, fails to mention provenance, or does not use any copied source files at all.
...
#### Imported: Troubleshooting Quick Reference
...
| --- | --- | --- |
| `references` | copied reference notes, guides, or background material from upstream | `references/aws-architecture.md` |
| `examples` | worked examples or reusable prompts copied from upstream | `examples/n/a` |
| `scripts` | upstream helper scripts that change execution or validation | `scripts/batch.mjs` |
| `agents` | routing or delegation notes that are genuinely part of the imported package | `agents/n/a` |
| `assets` | supporting assets or schemas copied from the source package | `assets/puppeteer-config.json` |

- [aws-architecture.md](references/aws-architecture.md)
- [c4-architecture.md](references/c4-architecture.md)
- [code-to-diagram.md](references/code-to-diagram.md)
- [diagram-types.md](references/diagram-types.md)
- [batch.mjs](scripts/batch.mjs)
- [render-ascii.mjs](scripts/render-ascii.mjs)
- [render.mjs](scripts/render.mjs)
- [setup.sh](scripts/setup.sh)

### Imported Reference Notes

#### Imported: C4 Diagrams — Mandatory Styling Guide
