...
version: "0.0.1"
category: devops
tags: ["mise", "devops", "ci-cd", "toolchain", "runtimes", "automation", "mise-configurator", "generate"]
complexity: beginner
risk: caution
...
```text
Use @mise-configurator to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @mise-configurator against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @mise-configurator for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @mise-configurator using the copied upstream files plus provenance, then summarize any gaps before merge.
```
