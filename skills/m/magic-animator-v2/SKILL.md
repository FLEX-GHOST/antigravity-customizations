---
name: magic-animator-v2
description: "Magic Animator Skill workflow skill. Use this skill when the user needs AI-powered animation tool for creating motion in logos, UI, icons, and social media assets and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: frontend
tags: ["magic-animator-v2", "magic-animator", "ai-powered", "animation", "tool", "for", "creating", "motion"]
complexity: beginner
risk: safe
...
# Magic Animator Skill Magic Animator enables designers to add life to static designs in seconds using AI-driven motion, transforming flat UX into premium, dynamic experiences.
...
- Use when the request clearly matches the imported source intent: AI-powered animation tool for creating motion in logos, UI, icons, and social media assets.
...
```text
Use @magic-animator-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @magic-animator-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @magic-animator-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @magic-animator-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/magic-animator`, fails to mention provenance, or does not use any copied source files at all.
