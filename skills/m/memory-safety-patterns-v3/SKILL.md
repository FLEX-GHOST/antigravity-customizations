---
name: memory-safety-patterns-v3
description: "Memory Safety Patterns workflow skill. Use this skill when the user needs Cross-language patterns for memory-safe programming including RAII, ownership, smart pointers, and resource management and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: development
tags: ["memory-safety-patterns-v3", "memory-safety-patterns", "cross-language", "patterns", "for", "memory-safe", "programming", "including"]
complexity: beginner
risk: safe
...
```text
Use @memory-safety-patterns-v3 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @memory-safety-patterns-v3 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @memory-safety-patterns-v3 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @memory-safety-patterns-v3 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-bundle-systems-programming/skills/memory-safety-patterns`, fails to mention provenance, or does not use any copied source files at all.
