...
version: "0.0.1"
category: ai-agents
tags: ["m365-agents-ts", "microsoft", "sdk", "for", "typescript", "node", "ai-agents"]
...
```text
Use @m365-agents-ts to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @m365-agents-ts against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @m365-agents-ts for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @m365-agents-ts using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/m365-agents-ts`, fails to mention provenance, or does not use any copied source files at all.
