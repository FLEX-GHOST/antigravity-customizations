...
| Workflow execution | `references/directories-timing-mistakes.md` | Starts with the smallest copied file that materially changes execution |
...
```text
Use @multi-platform-launch to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @multi-platform-launch against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @multi-platform-launch for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @multi-platform-launch using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `packages/skills-catalog/skills/(gtm)/multi-platform-launch`, fails to mention provenance, or does not use any copied source files at all.
...
| --- | --- | --- |
| `references` | copied reference notes, guides, or background material from upstream | `references/directories-timing-mistakes.md` |
| `examples` | worked examples or reusable prompts copied from upstream | `examples/n/a` |
| `scripts` | upstream helper scripts that change execution or validation | `scripts/n/a` |
| `agents` | routing or delegation notes that are genuinely part of the imported package | `agents/n/a` |
| `assets` | supporting assets or schemas copied from the source package | `assets/n/a` |

- [directories-timing-mistakes.md](references/directories-timing-mistakes.md)
