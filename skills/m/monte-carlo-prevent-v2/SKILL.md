...
version: "0.0.1"
category: data-ai
tags: ["data-observability", "dbt", "schema", "monte-carlo", "lineage", "monte-carlo-prevent-v2", "monte-carlo-prevent", "surfaces"]
complexity: advanced
risk: caution
...
| Workflow execution | `references/TROUBLESHOOTING.md` | Starts with the smallest copied file that materially changes execution |
| Supporting context | `references/parameters.md` | Adds the next most relevant copied source file without loading the entire package |
...
```text
Use @monte-carlo-prevent-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @monte-carlo-prevent-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @monte-carlo-prevent-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @monte-carlo-prevent-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/monte-carlo-prevent`, fails to mention provenance, or does not use any copied source files at all.
...
| --- | --- | --- |
| `references` | copied reference notes, guides, or background material from upstream | `references/TROUBLESHOOTING.md` |
