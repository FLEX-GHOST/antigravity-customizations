---
name: monte-carlo-validation-notebook-v2
description: "Setup workflow skill. Use this skill when the user needs Generates SQL validation notebooks for dbt PR changes with before/after comparison queries and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: data-ai
tags: ["data-observability", "validation", "dbt", "monte-carlo", "sql-notebook", "monte-carlo-validation-notebook-v2", "monte-carlo-validation-notebook", "generates"]
complexity: advanced
risk: caution
...
```text
Use @monte-carlo-validation-notebook-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @monte-carlo-validation-notebook-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @monte-carlo-validation-notebook-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @monte-carlo-validation-notebook-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/monte-carlo-validation-notebook`, fails to mention provenance, or does not use any copied source files at all.
...
| `scripts` | upstream helper scripts that change execution or validation | `scripts/generate_notebook_url.py` |
| `agents` | routing or delegation notes that are genuinely part of the imported package | `agents/n/a` |
| `assets` | supporting assets or schemas copied from the source package | `assets/n/a` |

- [generate_notebook_url.py](scripts/generate_notebook_url.py)
- [resolve_dbt_schema.py](scripts/resolve_dbt_schema.py)
- [generate_notebook_url.py](scripts/generate_notebook_url.py)
- [resolve_dbt_schema.py](scripts/resolve_dbt_schema.py)
