...
version: "0.0.1"
category: ai-agents
tags: ["langgraph", "langchain", "multi-agent", "orchestration", "deepagents", "rag", "tool-calling", "multi-agent-architect"]
complexity: advanced
risk: caution
...
```text
Use @multi-agent-architect to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @multi-agent-architect against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @multi-agent-architect for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @multi-agent-architect using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/multi-agent-architect`, fails to mention provenance, or does not use any copied source files at all.
