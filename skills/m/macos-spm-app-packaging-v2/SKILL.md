---
name: macos-spm-app-packaging-v2
...
tags: ["macos-spm-app-packaging-v2", "macos-spm-app-packaging", "scaffold", "build", "sign", "and", "package", "swiftpm"]
complexity: advanced
risk: caution
...
```text
Use @macos-spm-app-packaging-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @macos-spm-app-packaging-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @macos-spm-app-packaging-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @macos-spm-app-packaging-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/macos-spm-app-packaging`, fails to mention provenance, or does not use any copied source files at all.
