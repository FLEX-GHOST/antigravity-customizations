...
tags: ["playwright-skill-v3", "playwright-skill", "important", "path", "resolution", "can", "installed", "different"]
complexity: advanced
risk: caution
...
| Workflow execution | `API_REFERENCE.md` | Starts with the smallest copied file that materially changes execution |
| Supporting context | `lib/helpers.js` | Adds the next most relevant copied source file without loading the entire package |
...
```text
Use @playwright-skill-v3 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @playwright-skill-v3 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @playwright-skill-v3 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @playwright-skill-v3 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/playwright-skill`, fails to mention provenance, or does not use any copied source files at all.
