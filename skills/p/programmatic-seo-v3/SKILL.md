...
version: "0.0.1"
category: business
tags: ["programmatic-seo-v3", "programmatic-seo", "design", "and", "evaluate", "programmatic", "seo", "strategies"]
complexity: advanced
risk: safe
...
```text
Use @programmatic-seo-v3 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @programmatic-seo-v3 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @programmatic-seo-v3 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @programmatic-seo-v3 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-bundle-marketing-growth/skills/programmatic-seo`, fails to mention provenance, or does not use any copied source files at all.
