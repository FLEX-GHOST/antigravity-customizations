...
version: "0.0.1"
category: tools
tags: ["education", "puzzle", "classroom", "activity-planning", "event", "puzzle-activity-planner", "plan", "puzzle-based"]
complexity: beginner
risk: safe
...
```text
Use @puzzle-activity-planner to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @puzzle-activity-planner against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @puzzle-activity-planner for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @puzzle-activity-planner using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/puzzle-activity-planner`, fails to mention provenance, or does not use any copied source files at all.
