---
name: prometheus-configuration-v2
...
version: "0.0.1"
category: tools
tags: ["prometheus-configuration-v2", "prometheus-configuration", "complete", "guide", "prometheus", "setup", "metric", "collection"]
complexity: advanced
risk: caution
...
```text
Use @prometheus-configuration-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @prometheus-configuration-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @prometheus-configuration-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @prometheus-configuration-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/prometheus-configuration`, fails to mention provenance, or does not use any copied source files at all.
