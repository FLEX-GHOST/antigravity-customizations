---
name: product-marketing-context-v2
...
version: "0.0.1"
category: ai-agents
tags: ["product-marketing-context-v2", "product-marketing-context", "create", "update", "reusable", "product", "marketing", "context"]
complexity: advanced
risk: safe
...
```text
Use @product-marketing-context-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @product-marketing-context-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @product-marketing-context-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @product-marketing-context-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/product-marketing-context`, fails to mention provenance, or does not use any copied source files at all.
