---
name: python-testing-patterns-v3
description: "Python Testing Patterns workflow skill. Use this skill when the user needs Implement comprehensive testing strategies with pytest, fixtures, mocking, and test-driven development. Use when writing Python tests, setting up test suites, or implementing testing best practices and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: testing-security
tags: ["python-testing-patterns-v3", "python-testing-patterns", "implement", "comprehensive", "testing", "strategies", "pytest", "fixtures"]
complexity: beginner
risk: safe
...
```text
Use @python-testing-patterns-v3 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @python-testing-patterns-v3 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @python-testing-patterns-v3 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @python-testing-patterns-v3 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-bundle-python-pro/skills/python-testing-patterns`, fails to mention provenance, or does not use any copied source files at all.
