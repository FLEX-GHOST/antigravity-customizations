---
name: pc-games-v2
description: "PC/Console Game Development workflow skill. Use this skill when the user needs PC and console game development principles. Engine selection, platform features, optimization strategies and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: development
tags: ["pc-games-v2", "pc-games", "and", "console", "game", "development", "principles", "engine"]
complexity: beginner
risk: safe
...
```text
Use @pc-games-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @pc-games-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @pc-games-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @pc-games-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/game-development/pc-games`, fails to mention provenance, or does not use any copied source files at all.
