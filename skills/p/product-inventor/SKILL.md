...
version: "0.0.1"
category: design
tags: ["product-thinking", "innovation", "ux-design", "storytelling", "product-inventor", "product", "inventor", "design"]
complexity: advanced
risk: caution
...
```text
Use @product-inventor to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @product-inventor against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @product-inventor for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @product-inventor using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/product-inventor`, fails to mention provenance, or does not use any copied source files at all.
