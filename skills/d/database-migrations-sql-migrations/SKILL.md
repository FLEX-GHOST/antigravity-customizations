...
version: "0.0.1"
category: backend
tags: ["database-migrations-sql-migrations", "sql", "database", "migrations", "zero-downtime", "strategies", "for", "postgresql"]
complexity: beginner
risk: caution
...
```text
Use @database-migrations-sql-migrations to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @database-migrations-sql-migrations against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @database-migrations-sql-migrations for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @database-migrations-sql-migrations using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/database-migrations-sql-migrations`, fails to mention provenance, or does not use any copied source files at all.
