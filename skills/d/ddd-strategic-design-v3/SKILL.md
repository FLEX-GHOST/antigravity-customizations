...
version: "0.0.1"
category: design
tags: ["ddd", "strategic-design", "bounded-context", "ubiquitous-language", "ddd-strategic-design-v3", "ddd-strategic-design", "design", "strategic"]
complexity: beginner
risk: safe
...
```text
Use @ddd-strategic-design-v3 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @ddd-strategic-design-v3 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @ddd-strategic-design-v3 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @ddd-strategic-design-v3 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-bundle-ddd-evented-architecture/skills/ddd-strategic-design`, fails to mention provenance, or does not use any copied source files at all.
