...
tags: ["documentation-generation-doc-generate", "you", "are", "documentation", "expert", "specializing", "creating", "comprehensive"]
complexity: beginner
risk: safe
...
```text
Use @documentation-generation-doc-generate to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @documentation-generation-doc-generate against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @documentation-generation-doc-generate for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @documentation-generation-doc-generate using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/documentation-generation-doc-generate`, fails to mention provenance, or does not use any copied source files at all.
