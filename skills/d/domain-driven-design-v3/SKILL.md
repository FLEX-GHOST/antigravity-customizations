---
name: domain-driven-design-v3
description: "Domain-Driven Design workflow skill. Use this skill when the user needs Plan and route Domain-Driven Design work from strategic modeling to tactical implementation and evented architecture patterns and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: development
tags: ["ddd", "domain", "bounded-context", "architecture", "domain-driven-design-v3", "domain-driven-design", "plan", "and"]
complexity: beginner
risk: safe
...
Imported source sections that did not map cleanly to the public headings are still preserved below or in the support files. Notable imported sections: Output requirements, Limitations.
...
| Workflow execution | `references/ddd-deliverables.md` | Starts with the smallest copied file that materially changes execution |
| Supporting context | `references/ddd-deliverables.md` | Adds the next most relevant copied source file without loading the entire package |
...
5. Business rules are complex or fast-changing.
6. Multiple teams are causing model collisions.
7. Integration contracts are unstable.
...
```text
Use @domain-driven-design-v3 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @domain-driven-design-v3 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @domain-driven-design-v3 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @domain-driven-design-v3 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-bundle-ddd-evented-architecture/skills/domain-driven-design`, fails to mention provenance, or does not use any copied source files at all.
...
| --- | --- | --- |
| `references` | copied reference notes, guides, or background material from upstream | `references/ddd-deliverables.md` |
| `examples` | worked examples or reusable prompts copied from upstream | `examples/n/a` |
| `scripts` | upstream helper scripts that change execution or validation | `scripts/n/a` |
| `agents` | routing or delegation notes that are genuinely part of the imported package | `agents/n/a` |
| `assets` | supporting assets or schemas copied from the source package | `assets/n/a` |

- [ddd-deliverables.md](references/ddd-deliverables.md)
- [ddd-deliverables.md](references/ddd-deliverables.md)
