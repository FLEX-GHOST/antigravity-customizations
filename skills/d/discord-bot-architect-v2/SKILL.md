---
name: discord-bot-architect-v2
description: "Discord Bot Architect workflow skill. Use this skill when the user needs Specialized skill for building production-ready Discord bots and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: frontend
tags: ["discord-bot-architect-v2", "discord-bot-architect", "specialized", "for", "building", "production-ready", "discord", "bots"]
complexity: advanced
risk: caution
...
6. Use guild commands during development (instant updates)
7. Only deploy global commands when ready for production
...
#### Imported: Step 2: Request in code
...
```text
Use @discord-bot-architect-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @discord-bot-architect-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @discord-bot-architect-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @discord-bot-architect-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
#### Imported: Development: Use guild commands
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/discord-bot-architect`, fails to mention provenance, or does not use any copied source files at all.
...
#### Imported: Use environment variables properly
...
#### Imported: Generate correct invite URL
...
#### Imported: In Discord Developer Portal
...
#### Imported: Re-invite without kicking
...
#### Imported: Never block the event loop
...
#### Imported: Handle reconnections gracefully
...
#### Imported: Implement sharding at scale
...
#### Imported: Show modal immediately
...
#### Imported: If you need to check something first
