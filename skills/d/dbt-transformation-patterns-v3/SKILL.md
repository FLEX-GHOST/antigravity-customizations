---
name: dbt-transformation-patterns-v3
description: "dbt Transformation Patterns workflow skill. Use this skill when the user needs Production-ready patterns for dbt (data build tool) including model organization, testing strategies, documentation, and incremental processing and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: data-ai
tags: ["dbt-transformation-patterns-v3", "dbt-transformation-patterns", "production-ready", "patterns", "for", "dbt", "data", "build"]
complexity: beginner
risk: caution
...
1. Define model layers, naming, and ownership.
2. Implement tests, documentation, and freshness checks.
3. Choose materializations and incremental strategies.
4. Optimize runs with selectors and CI workflows.
...
```text
Use @dbt-transformation-patterns-v3 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @dbt-transformation-patterns-v3 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @dbt-transformation-patterns-v3 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @dbt-transformation-patterns-v3 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-bundle-data-engineering/skills/dbt-transformation-patterns`, fails to mention provenance, or does not use any copied source files at all.
