...
version: "0.0.1"
category: data-ai
tags: ["data-engineer", "build", "scalable", "data", "pipelines", "modern", "warehouses", "and"]
complexity: advanced
risk: caution
...
```text
Use @data-engineer to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @data-engineer against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @data-engineer for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @data-engineer using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/data-engineer`, fails to mention provenance, or does not use any copied source files at all.
