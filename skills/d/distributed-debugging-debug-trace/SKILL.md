...
tags: ["distributed-debugging-debug-trace", "you", "are", "debugging", "expert", "specializing", "setting", "comprehensive"]
complexity: beginner
risk: caution
...
```text
Use @distributed-debugging-debug-trace to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @distributed-debugging-debug-trace against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @distributed-debugging-debug-trace for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @distributed-debugging-debug-trace using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/distributed-debugging-debug-trace`, fails to mention provenance, or does not use any copied source files at all.
