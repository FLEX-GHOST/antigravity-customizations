...
version: "0.0.1"
category: devops
tags: ["event-sourcing-architect", "expert", "event", "sourcing", "cqrs", "and", "event-driven", "architecture"]
complexity: beginner
risk: caution
...
```text
Use @event-sourcing-architect to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @event-sourcing-architect against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @event-sourcing-architect for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @event-sourcing-architect using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/event-sourcing-architect`, fails to mention provenance, or does not use any copied source files at all.
