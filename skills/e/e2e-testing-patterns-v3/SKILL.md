---
name: e2e-testing-patterns-v3
description: "E2E Testing Patterns workflow skill. Use this skill when the user needs Build reliable, fast, and maintainable end-to-end test suites that provide confidence to ship code quickly and catch regressions before users do and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: testing-security
tags: ["e2e-testing-patterns-v3", "e2e-testing-patterns", "build", "reliable", "fast", "and", "maintainable", "end-to-end"]
complexity: beginner
risk: caution
...
```text
Use @e2e-testing-patterns-v3 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @e2e-testing-patterns-v3 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @e2e-testing-patterns-v3 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @e2e-testing-patterns-v3 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-bundle-qa-testing/skills/e2e-testing-patterns`, fails to mention provenance, or does not use any copied source files at all.
