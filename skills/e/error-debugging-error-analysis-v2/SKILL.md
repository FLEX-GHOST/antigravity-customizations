---
name: error-debugging-error-analysis-v2
...
tags: ["error-debugging-error-analysis-v2", "error-debugging-error-analysis", "you", "are", "expert", "error", "analysis", "specialist"]
complexity: beginner
risk: caution
...
```text
Use @error-debugging-error-analysis-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @error-debugging-error-analysis-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @error-debugging-error-analysis-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @error-debugging-error-analysis-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/error-debugging-error-analysis`, fails to mention provenance, or does not use any copied source files at all.
