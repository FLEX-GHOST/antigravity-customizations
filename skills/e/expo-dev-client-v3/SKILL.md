...
tags: ["expo-dev-client-v3", "expo-dev-client", "build", "and", "distribute", "expo", "development", "clients"]
...
```text
Use @expo-dev-client-v3 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @expo-dev-client-v3 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @expo-dev-client-v3 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @expo-dev-client-v3 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-bundle-expo-react-native/skills/expo-dev-client`, fails to mention provenance, or does not use any copied source files at all.
