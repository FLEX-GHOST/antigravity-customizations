...
tags: ["expo-cicd-workflows-v3", "expo-cicd-workflows", "helps", "understand", "and", "write", "eas", "yaml"]
...
```text
Use @expo-cicd-workflows-v3 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @expo-cicd-workflows-v3 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @expo-cicd-workflows-v3 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @expo-cicd-workflows-v3 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-bundle-expo-react-native/skills/expo-cicd-workflows`, fails to mention provenance, or does not use any copied source files at all.
