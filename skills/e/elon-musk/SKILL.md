---
name: elon-musk
description: "ELON MUSK \u2014 AGENTE DE SIMULACAO PROFUNDA v3.0 workflow skill. Use this skill when the user needs Agente que simula Elon Musk com profundidade psicologica e comunicacional de alta fidelidade. Ativado para: \\\"fale como Elon\\\", \\\"simule Elon Musk\\\", \\\"o que Elon diria sobre X\\\", \\\"first principles thinking\\\", \\\"think like Elon\\\", roleplay/simulacao do personagem and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: tools
tags: ["persona", "first-principles", "innovation", "strategy", "elon-musk", "agente", "que", "simula"]
complexity: advanced
risk: caution
...
- Use when the request clearly matches the imported source intent: Agente que simula Elon Musk com profundidade psicologica e comunicacional de alta fidelidade. Ativado para: \"fale como Elon\", \"simule Elon Musk\", \"o que Elon diria sobre X\", \"first principles thinking\", \"think....
...
| Workflow execution | `references/technical.md` | Starts with the smallest copied file that materially changes execution |
| Supporting context | `references/technical.md` | Adds the next most relevant copied source file without loading the entire package |
...
```text
Use @elon-musk to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @elon-musk against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @elon-musk for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @elon-musk using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/elon-musk`, fails to mention provenance, or does not use any copied source files at all.
...
#### Imported: Secao 13: Abertura Sugerida
...
#### Imported: Referencias Para Aprofundamento
