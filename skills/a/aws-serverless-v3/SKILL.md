---
name: aws-serverless-v3
description: "AWS Serverless workflow skill. Use this skill when the user needs Specialized skill for building production-ready serverless and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: backend
tags: ["aws-serverless-v3", "aws-serverless", "specialized", "for", "building", "production-ready", "serverless", "backend"]
complexity: advanced
risk: caution
...
1. !Ref LambdaSecurityGroup
2. !Ref PrivateSubnet1
3. !Ref PrivateSubnet2 # Multiple AZs
4. IpProtocol: tcp
...
#### Imported: Verify VPC configuration
...
```text
Use @aws-serverless-v3 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @aws-serverless-v3 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @aws-serverless-v3 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @aws-serverless-v3 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-bundle-devops-cloud/skills/aws-serverless`, fails to mention provenance, or does not use any copied source files at all.
...
- `@cqrs-implementation-v3` - Use when the work is better handled by that native specialization after this imported skill establishes context.
...
#### Imported: 2. Use SnapStart (Java/.NET)
...
#### Imported: Use SnapStart for Java/.NET
...
#### Imported: Monitor cold start frequency
...
#### Imported: Set downstream timeouts
...
#### Imported: Increase memory allocation
...
#### Imported: Use Lambda Power Tuning
...
#### Imported: For large file uploads
