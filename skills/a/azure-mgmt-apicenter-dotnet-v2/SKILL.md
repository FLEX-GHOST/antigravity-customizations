---
name: azure-mgmt-apicenter-dotnet-v2
description: "Azure.ResourceManager.ApiCenter (.NET) workflow skill. Use this skill when the user needs Azure API Center SDK for .NET. Centralized API inventory management with governance, versioning, and discovery and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: backend
tags: ["azure-mgmt-apicenter-dotnet-v2", "azure-mgmt-apicenter-dotnet", "azure", "api", "center", "sdk", "for", "net"]
complexity: advanced
risk: caution
...
```text
Use @azure-mgmt-apicenter-dotnet-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @azure-mgmt-apicenter-dotnet-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @azure-mgmt-apicenter-dotnet-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @azure-mgmt-apicenter-dotnet-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/azure-mgmt-apicenter-dotnet`, fails to mention provenance, or does not use any copied source files at all.
