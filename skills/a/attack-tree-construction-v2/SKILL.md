---
name: attack-tree-construction-v2
...
tags: ["attack-tree-construction-v2", "attack-tree-construction", "build", "comprehensive", "attack", "trees", "visualize", "threat"]
complexity: beginner
risk: caution
...
> AUTHORIZED USE ONLY: Use this skill only for authorized security assessments, defensive validation, or controlled educational environments. # Attack Tree Construction Systematic attack path visualization and analysis.
...
1. Confirm scope, assets, and the attacker goal for the root node.
2. Decompose into sub-goals with AND/OR structure.
3. Annotate leaves with cost, skill, time, and detectability.
4. Map mitigations per branch and prioritize high-impact paths.
...
```text
Use @attack-tree-construction-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @attack-tree-construction-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @attack-tree-construction-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @attack-tree-construction-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/attack-tree-construction`, fails to mention provenance, or does not use any copied source files at all.
