---
name: anti-reversing-techniques-v2
description: "anti-reversing-techniques workflow skill. Use this skill when the user needs AUTHORIZED USE ONLY: This skill contains dual-use security techniques. Before proceeding with any bypass or analysis: > 1 and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: testing-security
tags: ["anti-reversing-techniques-v2", "anti-reversing-techniques", "authorized", "use", "only", "contains", "dual-use", "security"]
complexity: beginner
risk: caution
...
```text
Use @anti-reversing-techniques-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @anti-reversing-techniques-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @anti-reversing-techniques-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @anti-reversing-techniques-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/anti-reversing-techniques`, fails to mention provenance, or does not use any copied source files at all.
