...
tags: ["awt-e2e-testing", "ai-powered", "e2e", "web", "testing", "eyes", "and", "hands"]
complexity: beginner
risk: safe
...
```text
Use @awt-e2e-testing to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @awt-e2e-testing against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @awt-e2e-testing for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @awt-e2e-testing using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/awt-e2e-testing`, fails to mention provenance, or does not use any copied source files at all.
