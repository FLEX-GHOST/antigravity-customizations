---
name: azure-ai-voicelive-dotnet-v2
...
version: "0.0.1"
category: backend
tags: ["azure-ai-voicelive-dotnet-v2", "azure-ai-voicelive-dotnet", "azure", "voice", "live", "sdk", "for", "net"]
...
```text
Use @azure-ai-voicelive-dotnet-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @azure-ai-voicelive-dotnet-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @azure-ai-voicelive-dotnet-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @azure-ai-voicelive-dotnet-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/azure-ai-voicelive-dotnet`, fails to mention provenance, or does not use any copied source files at all.
