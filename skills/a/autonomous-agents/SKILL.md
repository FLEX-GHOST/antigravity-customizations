...
description: "Autonomous Agents workflow skill. Use this skill when the user needs Autonomous agents are AI systems that can independently decompose and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: ai-agents
tags: ["autonomous-agents", "autonomous", "are", "systems", "can", "independently", "decompose", "ai-agents"]
complexity: advanced
risk: caution
...
```text
Use @autonomous-agents to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @autonomous-agents against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @autonomous-agents for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @autonomous-agents using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/autonomous-agents`, fails to mention provenance, or does not use any copied source files at all.
