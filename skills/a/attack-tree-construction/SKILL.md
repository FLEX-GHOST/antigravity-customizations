...
tags: ["attack-tree-construction", "build", "comprehensive", "attack", "trees", "visualize", "threat", "paths"]
complexity: beginner
risk: caution
...
```text
Use @attack-tree-construction to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @attack-tree-construction against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @attack-tree-construction for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @attack-tree-construction using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/attack-tree-construction`, fails to mention provenance, or does not use any copied source files at all.
