---
name: advogado-especialista-v2
description: "ADVOGADO ESPECIALISTA ELITE \u2014 JURISTA COMPLETO workflow skill. Use this skill when the user needs Advogado especialista em todas as areas do Direito brasileiro: familia, criminal, trabalhista, tributario, consumidor, imobiliario, empresarial, civil e constitucional and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: devops
tags: ["legal", "brazilian-law", "multi-domain", "portuguese", "advogado-especialista-v2", "advogado-especialista", "advogado", "especialista"]
complexity: advanced
risk: caution
...
| Supporting context | `references/fontes.md` | Adds the next most relevant copied source file without loading the entire package |
...
#### Imported: Etapa 6 — Riscos Processuais
...
```text
Use @advogado-especialista-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @advogado-especialista-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @advogado-especialista-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @advogado-especialista-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/advogado-especialista`, fails to mention provenance, or does not use any copied source files at all.
...
#### Imported: 1. Identificar A Area Do Direito
...
#### Imported: 2. Identificar O Perfil Do Cliente
...
#### Imported: Modulo 2 — Direito Criminal E Penal (Resumo Executivo)
...
#### Imported: Modulo 3 — Maria Da Penha (Resumo Executivo)
...
#### Imported: Etapa 10 — Prazos Relevantes
...
#### Imported: Etapa 11 — Medidas De Urgencia
...
#### Imported: Etapa 12 — Parecer Final
...
#### Imported: Juizados Especiais (Lei 9.099/1995)
...
#### Imported: Stj — Responsabilidade Civil
...
#### Imported: Orquestracao Com Outros Skills
...
#### Imported: Verificar Se A Skill Esta Registrada:
...
#### Imported: Via Orchestrator (Automatico):

python agent-orchestrator/scripts/match_skills.py "preciso de um advogado"

#### Imported: "Quero Fazer Partilha De Bens"
