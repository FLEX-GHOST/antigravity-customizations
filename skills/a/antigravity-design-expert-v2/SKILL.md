---
name: antigravity-design-expert-v2
description: "Antigravity UI & Motion Design Expert workflow skill. Use this skill when the user needs Core UI/UX engineering skill for building highly interactive, spatial, weightless, and glassmorphism-based web interfaces using GSAP and 3D CSS and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: frontend
tags: ["antigravity-design-expert-v2", "antigravity-design-expert", "core", "engineering", "for", "building", "highly", "interactive"]
complexity: beginner
risk: safe
...
Imported source sections that did not map cleanly to the public headings are still preserved below or in the support files. Notable imported sections: 🎯 Role Overview, 🛠️ Preferred Tech Stack, 🚧 Execution Constraints, Limitations.
...
- Use when the request clearly matches the imported source intent: Core UI/UX engineering skill for building highly interactive, spatial, weightless, and glassmorphism-based web interfaces using GSAP and 3D CSS.
...
```text
Use @antigravity-design-expert-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @antigravity-design-expert-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @antigravity-design-expert-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @antigravity-design-expert-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
#### Imported: 📐 Design Principles (The "Antigravity" Vibe)
...
#### Imported: 🎬 Motion & Animation Rules
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/antigravity-design-expert`, fails to mention provenance, or does not use any copied source files at all.
...
#### Imported: 🛠️ Preferred Tech Stack
...
#### Imported: 🚧 Execution Constraints
