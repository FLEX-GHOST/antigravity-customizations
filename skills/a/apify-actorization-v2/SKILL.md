...
```text
Use @apify-actorization-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @apify-actorization-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @apify-actorization-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @apify-actorization-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/apify-actorization`, fails to mention provenance, or does not use any copied source files at all.
...
- [cli-actorization.md](references/cli-actorization.md)
- [js-ts-actorization.md](references/js-ts-actorization.md)
- [python-actorization.md](references/python-actorization.md)
- [schemas-and-output.md](references/schemas-and-output.md)
- [cli-actorization.md](references/cli-actorization.md)
- [js-ts-actorization.md](references/js-ts-actorization.md)
