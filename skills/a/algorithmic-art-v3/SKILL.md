...
| Supporting context | `templates/generator_template.js` | Adds the next most relevant copied source file without loading the entire package |
...
7. Seed navigation (always present)
...
```text
Use @algorithmic-art-v3 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @algorithmic-art-v3 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @algorithmic-art-v3 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @algorithmic-art-v3 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-bundle-creative-director/skills/algorithmic-art`, fails to mention provenance, or does not use any copied source files at all.
...
- `@azd-deployment-v3` - Use when the work is better handled by that native specialization after this imported skill establishes context.
...
- [generator_template.js](templates/generator_template.js)
- [viewer.html](templates/viewer.html)
