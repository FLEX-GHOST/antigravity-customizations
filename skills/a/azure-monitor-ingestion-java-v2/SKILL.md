---
name: azure-monitor-ingestion-java-v2
...
version: "0.0.1"
category: data-ai
tags: ["azure-monitor-ingestion-java-v2", "azure-monitor-ingestion-java", "azure", "monitor", "ingestion", "sdk", "for", "java"]
...
```text
Use @azure-monitor-ingestion-java-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @azure-monitor-ingestion-java-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @azure-monitor-ingestion-java-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @azure-monitor-ingestion-java-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
