---
name: azure-resource-manager-cosmosdb-dotnet-v2
...
tags: ["azure-resource-manager-cosmosdb-dotnet-v2", "azure-resource-manager-cosmosdb-dotnet", "azure", "resource", "manager", "sdk", "for", "cosmos"]
...
```text
Use @azure-resource-manager-cosmosdb-dotnet-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @azure-resource-manager-cosmosdb-dotnet-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @azure-resource-manager-cosmosdb-dotnet-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @azure-resource-manager-cosmosdb-dotnet-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/azure-resource-manager-cosmosdb-dotnet`, fails to mention provenance, or does not use any copied source files at all.
