...
version: "0.0.1"
category: backend
tags: ["apify-brand-reputation-monitoring", "scrape", "reviews", "ratings", "and", "brand", "mentions", "multiple"]
...
```text
Use @apify-brand-reputation-monitoring to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @apify-brand-reputation-monitoring against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @apify-brand-reputation-monitoring for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @apify-brand-reputation-monitoring using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/apify-brand-reputation-monitoring`, fails to mention provenance, or does not use any copied source files at all.
