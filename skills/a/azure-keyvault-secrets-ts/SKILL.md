...
description: "Azure Key Vault Secrets SDK for TypeScript workflow skill. Use this skill when the user needs Manage secrets using Azure Key Vault Secrets SDK for JavaScript (@azure/keyvault-secrets). Use when storing and retrieving application secrets or configuration values and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: cli-automation
tags: ["azure-keyvault-secrets-ts", "manage", "secrets", "using", "azure", "key", "vault", "sdk"]
...
```text
Use @azure-keyvault-secrets-ts to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @azure-keyvault-secrets-ts against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @azure-keyvault-secrets-ts for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @azure-keyvault-secrets-ts using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/azure-keyvault-secrets-ts`, fails to mention provenance, or does not use any copied source files at all.
