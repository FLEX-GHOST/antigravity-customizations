---
name: avoid-ai-writing-v2
description: "Avoid AI Writing \u2014 Audit & Rewrite workflow skill. Use this skill when the user needs Audit and rewrite content to remove 21 categories of AI writing patterns with a 43-entry replacement table and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: communication
tags: ["avoid-ai-writing-v2", "avoid-ai-writing", "audit", "and", "rewrite", "content", "remove", "categories"]
complexity: beginner
risk: safe
...
```text
Use @avoid-ai-writing-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @avoid-ai-writing-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @avoid-ai-writing-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @avoid-ai-writing-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/avoid-ai-writing`, fails to mention provenance, or does not use any copied source files at all.
