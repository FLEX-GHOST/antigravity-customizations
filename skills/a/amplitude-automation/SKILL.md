...
version: "0.0.1"
category: tools
tags: ["amplitude-automation", "automate", "amplitude", "tasks", "via", "rube", "mcp", "composio"]
complexity: advanced
risk: caution
...
```text
Use @amplitude-automation to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @amplitude-automation against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @amplitude-automation for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @amplitude-automation using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/amplitude-automation`, fails to mention provenance, or does not use any copied source files at all.
