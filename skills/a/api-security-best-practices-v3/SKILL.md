---
name: api-security-best-practices-v3
...
version: "0.0.1"
category: backend
tags: ["api-security-best-practices-v3", "api-security-best-practices", "implement", "secure", "api", "design", "patterns", "including"]
complexity: advanced
risk: caution
...
```text
Use @api-security-best-practices-v3 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @api-security-best-practices-v3 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @api-security-best-practices-v3 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @api-security-best-practices-v3 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-bundle-security-developer/skills/api-security-best-practices`, fails to mention provenance, or does not use any copied source files at all.
