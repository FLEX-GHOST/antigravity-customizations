---
name: avalonia-viewmodels-zafiro-v2
...
tags: ["avalonia-viewmodels-zafiro-v2", "avalonia-viewmodels-zafiro", "optimal", "viewmodel", "and", "wizard", "creation", "patterns"]
...
```text
Use @avalonia-viewmodels-zafiro-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @avalonia-viewmodels-zafiro-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @avalonia-viewmodels-zafiro-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @avalonia-viewmodels-zafiro-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/avalonia-viewmodels-zafiro`, fails to mention provenance, or does not use any copied source files at all.
