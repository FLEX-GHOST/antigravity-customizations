...
version: "0.0.1"
category: devops
tags: ["legal", "brazilian-law", "multi-domain", "portuguese", "advogado-especialista", "advogado", "especialista", "todas"]
complexity: advanced
risk: caution
...
```text
Use @advogado-especialista to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @advogado-especialista against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @advogado-especialista for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @advogado-especialista using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/advogado-especialista`, fails to mention provenance, or does not use any copied source files at all.
