...
version: "0.0.1"
category: backend
tags: ["azure-resource-manager-mysql-dotnet", "azure", "mysql", "flexible", "server", "sdk", "for", "net"]
complexity: advanced
risk: caution
...
```text
Use @azure-resource-manager-mysql-dotnet to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @azure-resource-manager-mysql-dotnet against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @azure-resource-manager-mysql-dotnet for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @azure-resource-manager-mysql-dotnet using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/azure-resource-manager-mysql-dotnet`, fails to mention provenance, or does not use any copied source files at all.
