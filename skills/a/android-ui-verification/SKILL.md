...
version: "0.0.1"
category: frontend
tags: ["android-ui-verification", "android", "verification", "automated", "end-to-end", "testing", "and", "emulator"]
complexity: beginner
risk: safe
...
```text
Use @android-ui-verification to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @android-ui-verification against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @android-ui-verification for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @android-ui-verification using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/android_ui_verification`, fails to mention provenance, or does not use any copied source files at all.
