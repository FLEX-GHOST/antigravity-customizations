...
version: "0.0.1"
category: data-ai
tags: ["analytics-tracking-v3", "analytics-tracking", "design", "audit", "and", "improve", "analytics", "tracking"]
...
```text
Use @analytics-tracking-v3 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @analytics-tracking-v3 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @analytics-tracking-v3 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @analytics-tracking-v3 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-bundle-data-analytics/skills/analytics-tracking`, fails to mention provenance, or does not use any copied source files at all.
