...
tags: ["algolia-search-v4", "algolia-search", "expert", "patterns", "for", "algolia", "search", "implementation"]
complexity: advanced
risk: caution
...
```text
Use @algolia-search-v4 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @algolia-search-v4 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @algolia-search-v4 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @algolia-search-v4 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-bundle-integration-apis/skills/algolia-search`, fails to mention provenance, or does not use any copied source files at all.
