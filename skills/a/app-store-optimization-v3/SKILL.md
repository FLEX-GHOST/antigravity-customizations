...
| Workflow execution | `HOW_TO_USE.md` | Starts with the smallest copied file that materially changes execution |
...
```text
Use @app-store-optimization-v3 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @app-store-optimization-v3 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @app-store-optimization-v3 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @app-store-optimization-v3 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-bundle-expo-react-native/skills/app-store-optimization`, fails to mention provenance, or does not use any copied source files at all.
...
- [README.md](README.md)
- [ab_test_planner.py](ab_test_planner.py)
- [aso_scorer.py](aso_scorer.py)
- [competitor_analyzer.py](competitor_analyzer.py)
