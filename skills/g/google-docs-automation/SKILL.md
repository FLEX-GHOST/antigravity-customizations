...
version: "0.0.1"
category: backend
tags: ["google-docs-automation", "lightweight", "google", "docs", "integration", "standalone", "oauth", "authentication"]
complexity: beginner
risk: caution
...
```text
Use @google-docs-automation to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @google-docs-automation against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @google-docs-automation for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @google-docs-automation using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/google-docs-automation`, fails to mention provenance, or does not use any copied source files at all.
