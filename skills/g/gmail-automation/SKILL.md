...
version: "0.0.1"
category: backend
tags: ["gmail-automation", "lightweight", "gmail", "integration", "standalone", "oauth", "authentication", "mcp"]
...
```text
Use @gmail-automation to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @gmail-automation against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @gmail-automation for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @gmail-automation using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/gmail-automation`, fails to mention provenance, or does not use any copied source files at all.
