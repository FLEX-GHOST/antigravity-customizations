...
description: "GEO Fundamentals workflow skill. Use this skill when the user needs Generative Engine Optimization for AI search engines (ChatGPT, Claude, Perplexity) and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: ai-agents
tags: ["geo-fundamentals", "generative", "engine", "optimization", "for", "search", "engines", "chatgpt"]
complexity: beginner
risk: safe
...
| Workflow execution | `scripts/geo_checker.py` | Starts with the smallest copied file that materially changes execution |
| Supporting context | `scripts/geo_checker.py` | Adds the next most relevant copied source file without loading the entire package |
...
```text
Use @geo-fundamentals to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @geo-fundamentals against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @geo-fundamentals for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @geo-fundamentals using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/geo-fundamentals`, fails to mention provenance, or does not use any copied source files at all.
...
| `scripts` | upstream helper scripts that change execution or validation | `scripts/geo_checker.py` |
| `agents` | routing or delegation notes that are genuinely part of the imported package | `agents/n/a` |
| `assets` | supporting assets or schemas copied from the source package | `assets/n/a` |

- [geo_checker.py](scripts/geo_checker.py)
- [geo_checker.py](scripts/geo_checker.py)
