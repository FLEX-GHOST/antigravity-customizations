---
name: gpt-taste-v2
description: "CORE DIRECTIVE: AWWWARDS-LEVEL DESIGN ENGINEERING workflow skill. Use this skill when the user needs generating elite GSAP-heavy frontend pages with strict AIDA structure, wide hero typography, and gapless bento grids and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: frontend
tags: ["frontend", "design", "gsap", "motion", "gpt-taste-v2", "gpt-taste", "generating", "elite"]
complexity: advanced
risk: safe
...
```text
Use @gpt-taste-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @gpt-taste-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @gpt-taste-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @gpt-taste-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/gpt-taste`, fails to mention provenance, or does not use any copied source files at all.
