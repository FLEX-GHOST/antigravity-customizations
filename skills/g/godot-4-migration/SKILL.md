...
version: "0.0.1"
category: tools
tags: ["godot-4-migration", "specialized", "guide", "for", "migrating", "godot", "projects", "gdscript"]
complexity: beginner
risk: caution
...
```text
Use @godot-4-migration to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @godot-4-migration against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @godot-4-migration for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @godot-4-migration using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/godot-4-migration`, fails to mention provenance, or does not use any copied source files at all.
