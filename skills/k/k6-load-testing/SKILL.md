...
tags: ["k6", "load-testing", "performance", "api-testing", "ci-cd", "k6-load-testing", "comprehensive", "load"]
complexity: advanced
risk: caution
...
```text
Use @k6-load-testing to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @k6-load-testing against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @k6-load-testing for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @k6-load-testing using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/k6-load-testing`, fails to mention provenance, or does not use any copied source files at all.
