---
name: kotler-macro-analyzer-v2
description: "Kotler Macro-Environment Analyzer workflow skill. Use this skill when the user needs Professional PESTEL/SWOT analysis agent based on Kotler's methodology for strategic market audits and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: testing-security
tags: ["marketing", "economics", "strategy", "kotler", "pestel", "kotler-macro-analyzer-v2", "kotler-macro-analyzer", "professional"]
complexity: beginner
risk: safe
...
- Use when the request clearly matches the imported source intent: Professional PESTEL/SWOT analysis agent based on Kotler's methodology for strategic market audits.
...
```text
Use @kotler-macro-analyzer-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @kotler-macro-analyzer-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @kotler-macro-analyzer-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @kotler-macro-analyzer-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/kotler-macro-analyzer`, fails to mention provenance, or does not use any copied source files at all.
