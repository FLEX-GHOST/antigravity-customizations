...
description: "Technical Design Doc Creator workflow skill. Use this skill when the user needs Creates comprehensive Technical Design Documents (TDD) with mandatory and optional sections through interactive discovery. Use when user asks to \"write a design doc\", \"create a TDD\", \"technical spec\", \"architecture document\", \"RFC\", \"design proposal\", or needs to document a technical decision before implementation. Do NOT use for README files, API docs, or general documentation (use docs-writer instead) and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: documentation
tags: ["technical-design-doc-creator", "creates", "comprehensive", "technical", "design", "documents", "tdd", "mandatory"]
complexity: advanced
risk: caution
...
1. Tech Lead / Owner
2. Team members
3. Problem description (what/why/impact)
4. What is in scope
...
```text
Use @technical-design-doc-creator to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @technical-design-doc-creator against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @technical-design-doc-creator for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @technical-design-doc-creator using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `packages/skills-catalog/skills/(creation)/create-technical-design-doc`, fails to mention provenance, or does not use any copied source files at all.
...
#### Imported: Problem Statement & Motivation
...
#### Imported: Industry Standards Reference
...
#### Imported: High-Level vs Implementation Details
...
#### Imported: Project Size Adaptation
...
#### Imported: Security Considerations
...
#### Imported: Monitoring & Observability
...
#### Imported: Alternatives Considered
...
#### Imported: Performance Requirements
