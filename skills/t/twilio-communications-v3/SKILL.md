---
name: twilio-communications-v3
description: "Twilio Communications workflow skill. Use this skill when the user needs \"Build communication features with Twilio: SMS messaging, voice and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: communication
tags: ["twilio-communications-v3", "twilio-communications", "build", "communication", "features", "twilio", "sms", "messaging"]
complexity: advanced
risk: caution
...
Imported source sections that did not map cleanly to the public headings are still preserved below or in the support files. Notable imported sections: Patterns, Sharp Edges, Track opt-out status in your database, Include opt-out instructions, Provide fallback channel, Register for A2P 10DLC (US requirement).
...
```text
Use @twilio-communications-v3 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @twilio-communications-v3 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @twilio-communications-v3 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @twilio-communications-v3 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-bundle-integration-apis/skills/twilio-communications`, fails to mention provenance, or does not use any copied source files at all.
...
#### Imported: Implement retry logic for transient failures
...
#### Imported: Track opt-out status in your database
...
#### Imported: Include opt-out instructions
...
#### Imported: Provide fallback channel
...
#### Imported: Register for A2P 10DLC (US requirement)
...
#### Imported: Message content best practices
...
#### Imported: Monitor delivery rates
...
#### Imported: ALWAYS validate the signature
...
#### Imported: Common validation gotchas
...
#### Imported: Track session windows per user
...
#### Imported: Incoming message webhook
...
#### Imported: Create approved templates for common messages
...
#### Imported: Never hardcode credentials
...
#### Imported: Use API Key instead of Auth Token
...
#### Imported: Rotate tokens immediately if exposed
...
#### Imported: Implement application-level rate limiting too
...
#### Imported: Provide clear user feedback
