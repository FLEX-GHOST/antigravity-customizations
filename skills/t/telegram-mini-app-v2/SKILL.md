---
name: telegram-mini-app-v2
description: "Telegram Mini App workflow skill. Use this skill when the user needs Expert in building Telegram Mini Apps (TWA) - web apps that run and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: fullstack-web
tags: ["telegram-mini-app-v2", "telegram-mini-app", "expert", "building", "telegram", "mini", "apps", "twa"]
complexity: advanced
risk: caution
...
```text
Use @telegram-mini-app-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @telegram-mini-app-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @telegram-mini-app-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @telegram-mini-app-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/telegram-mini-app`, fails to mention provenance, or does not use any copied source files at all.
