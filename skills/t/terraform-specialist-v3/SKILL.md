---
name: terraform-specialist-v3
description: "terraform-specialist workflow skill. Use this skill when the user needs Expert Terraform/OpenTofu specialist mastering advanced IaC automation, state management, and enterprise infrastructure patterns and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: devops
tags: ["terraform-specialist-v3", "terraform-specialist", "expert", "terraform", "opentofu", "specialist", "mastering", "advanced"]
complexity: advanced
risk: caution
...
```text
Use @terraform-specialist-v3 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @terraform-specialist-v3 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @terraform-specialist-v3 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @terraform-specialist-v3 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-bundle-devops-cloud/skills/terraform-specialist`, fails to mention provenance, or does not use any copied source files at all.
