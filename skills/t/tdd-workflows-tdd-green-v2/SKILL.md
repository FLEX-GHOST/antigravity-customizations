---
name: tdd-workflows-tdd-green-v2
description: "Green Phase: Simple function workflow skill. Use this skill when the user needs Implement the minimal code needed to make failing tests pass in the TDD green phase and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: development
tags: ["tdd-workflows-tdd-green-v2", "tdd-workflows-tdd-green", "implement", "the", "minimal", "needed", "make", "failing"]
complexity: beginner
risk: safe
...
```text
Use @tdd-workflows-tdd-green-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @tdd-workflows-tdd-green-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @tdd-workflows-tdd-green-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @tdd-workflows-tdd-green-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/tdd-workflows-tdd-green`, fails to mention provenance, or does not use any copied source files at all.
