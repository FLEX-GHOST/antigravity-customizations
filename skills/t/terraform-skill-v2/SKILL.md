---
name: terraform-skill-v2
description: "Terraform Skill for Claude workflow skill. Use this skill when the user needs Terraform infrastructure as code best practices and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: devops
tags: ["terraform-skill-v2", "terraform-skill", "terraform", "infrastructure", "best", "practices", "devops"]
complexity: advanced
risk: caution
...
```text
Use @terraform-skill-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @terraform-skill-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @terraform-skill-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @terraform-skill-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/terraform-skill`, fails to mention provenance, or does not use any copied source files at all.
