...
tags: ["tdd-workflows-tdd-green", "implement", "the", "minimal", "needed", "make", "failing", "tests"]
complexity: beginner
risk: safe
...
```text
Use @tdd-workflows-tdd-green to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @tdd-workflows-tdd-green against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @tdd-workflows-tdd-green for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @tdd-workflows-tdd-green using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/tdd-workflows-tdd-green`, fails to mention provenance, or does not use any copied source files at all.
