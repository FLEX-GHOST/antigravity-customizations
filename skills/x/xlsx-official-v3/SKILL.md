---
name: xlsx-official-v3
description: "Requirements for Outputs workflow skill. Use this skill when the user needs Unless otherwise stated by the user or existing template and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: cli-automation
tags: ["xlsx-official-v3", "xlsx-official", "unless", "otherwise", "stated", "the", "user", "existing"]
complexity: advanced
risk: caution
...
```text
Use @xlsx-official-v3 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @xlsx-official-v3 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @xlsx-official-v3 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @xlsx-official-v3 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-bundle-documents-presentations/skills/xlsx-official`, fails to mention provenance, or does not use any copied source files at all.
