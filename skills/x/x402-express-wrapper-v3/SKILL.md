...
tags: ["x402", "defi", "monetization", "express", "mcp", "x402-express-wrapper-v3", "x402-express-wrapper", "wrapper"]
complexity: beginner
risk: safe
...
Imported source sections that did not map cleanly to the public headings are still preserved below or in the support files. Notable imported sections: 📦 Instalación, 🛠️ Implementación, 🧠 Consideraciones Arquitectónicas (Agentic Context), Limitations.
...
```text
Use @x402-express-wrapper-v3 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @x402-express-wrapper-v3 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @x402-express-wrapper-v3 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @x402-express-wrapper-v3 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/x402-express-wrapper`, fails to mention provenance, or does not use any copied source files at all.
