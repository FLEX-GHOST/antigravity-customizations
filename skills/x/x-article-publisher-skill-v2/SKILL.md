---
name: x-article-publisher-skill-v2
description: "X Article Publisher Skill workflow skill. Use this skill when the user needs Publish articles to X/Twitter and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: testing-security
tags: ["x-article-publisher-skill-v2", "x-article-publisher-skill", "publish", "articles", "twitter", "testing-security"]
complexity: beginner
risk: safe
...
- Use when the request clearly matches the imported source intent: Publish articles to X/Twitter.
...
1. This skill provides guidance and patterns for publish articles to x/twitter.
...
```text
Use @x-article-publisher-skill-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @x-article-publisher-skill-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @x-article-publisher-skill-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @x-article-publisher-skill-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/x-article-publisher-skill`, fails to mention provenance, or does not use any copied source files at all.
