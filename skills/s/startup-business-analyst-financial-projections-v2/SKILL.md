---
name: startup-business-analyst-financial-projections-v2
description: "Financial Projections workflow skill. Use this skill when the user needs 'Create detailed 3-5 year financial model with revenue, costs, cash and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: devops
tags: ["startup-business-analyst-financial-projections-v2", "startup-business-analyst-financial-projections", "create", "detailed", "year", "financial", "model", "revenue"]
complexity: advanced
risk: safe
...
```text
Use @startup-business-analyst-financial-projections-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @startup-business-analyst-financial-projections-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @startup-business-analyst-financial-projections-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @startup-business-analyst-financial-projections-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/startup-business-analyst-financial-projections`, fails to mention provenance, or does not use any copied source files at all.
