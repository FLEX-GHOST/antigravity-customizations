---
name: service-mesh-expert-v2
description: "Service Mesh Expert workflow skill. Use this skill when the user needs Expert service mesh architect specializing in Istio, Linkerd, and cloud-native networking patterns. Masters traffic management, security policies, observability integration, and multi-cluster mesh con and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: devops
tags: ["service-mesh-expert-v2", "service-mesh-expert", "expert", "service", "mesh", "architect", "specializing", "istio"]
complexity: beginner
risk: caution
...
5. Assess current infrastructure and requirements
6. Design mesh topology and traffic policies
7. Implement security policies (mTLS, AuthorizationPolicy)
...
```text
Use @service-mesh-expert-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @service-mesh-expert-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @service-mesh-expert-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @service-mesh-expert-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/service-mesh-expert`, fails to mention provenance, or does not use any copied source files at all.
