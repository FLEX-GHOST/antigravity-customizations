---
name: security-compliance-compliance-check-v2
...
tags: ["security-compliance-compliance-check-v2", "security-compliance-compliance-check", "you", "are", "compliance", "expert", "specializing", "regulatory"]
complexity: beginner
risk: safe
...
# Regulatory Compliance Check You are a compliance expert specializing in regulatory requirements for software systems including GDPR, HIPAA, SOC2, PCI-DSS, and other industry standards. Perform comprehensive compliance audits and provide implementation guidance for achieving and maintaining compliance.
...
```text
Use @security-compliance-compliance-check-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @security-compliance-compliance-check-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @security-compliance-compliance-check-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @security-compliance-compliance-check-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/security-compliance-compliance-check`, fails to mention provenance, or does not use any copied source files at all.
