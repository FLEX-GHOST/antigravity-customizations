---
name: subagent-driven-development-v2
description: "Subagent-Driven Development workflow skill. Use this skill when the user needs executing implementation plans with independent tasks in the current session and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: development
tags: ["subagent-driven-development-v2", "subagent-driven-development", "executing", "implementation", "plans", "independent", "tasks", "the"]
complexity: advanced
risk: safe
...
```text
Use @subagent-driven-development-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @subagent-driven-development-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @subagent-driven-development-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @subagent-driven-development-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/subagent-driven-development`, fails to mention provenance, or does not use any copied source files at all.
