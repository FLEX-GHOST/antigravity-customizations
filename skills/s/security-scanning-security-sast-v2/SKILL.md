---
name: security-scanning-security-sast-v2
...
tags: ["security-scanning-security-sast-v2", "security-scanning-security-sast", "static", "application", "security", "testing", "sast", "for"]
complexity: advanced
risk: caution
...
```text
Use @security-scanning-security-sast-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @security-scanning-security-sast-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @security-scanning-security-sast-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @security-scanning-security-sast-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/security-scanning-security-sast`, fails to mention provenance, or does not use any copied source files at all.
