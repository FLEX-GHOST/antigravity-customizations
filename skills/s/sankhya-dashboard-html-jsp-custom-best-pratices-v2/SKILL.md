---
name: sankhya-dashboard-html-jsp-custom-best-pratices-v2
description: "sankhya-dashboard-html-jsp-custom-best-pratices workflow skill. Use this skill when the user needs This skill should be used when the user asks for patterns, best practices, creation, or fixing of Sankhya dashboards using HTML, JSP, Java, and SQL and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: frontend
tags: ["sankhya", "dashboard", "jsp", "html", "sql", "best-practices", "sankhya-dashboard-html-jsp-custom-best-pratices-v2", "sankhya-dashboard-html-jsp-custom-best-pratices"]
complexity: advanced
risk: caution
...
Imported source sections that did not map cleanly to the public headings are still preserved below or in the support files. Notable imported sections: Purpose, Core Capabilities, Patterns, Limitations.
...
- Use when the request clearly matches the imported source intent: This skill should be used when the user asks for patterns, best practices, creation, or fixing of Sankhya dashboards using HTML, JSP, Java, and SQL.
...
```text
Use @sankhya-dashboard-html-jsp-custom-best-pratices-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @sankhya-dashboard-html-jsp-custom-best-pratices-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @sankhya-dashboard-html-jsp-custom-best-pratices-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @sankhya-dashboard-html-jsp-custom-best-pratices-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/sankhya-dashboard-html-jsp-custom-best-pratices`, fails to mention provenance, or does not use any copied source files at all.
