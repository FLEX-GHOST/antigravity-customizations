---
name: seo-structure-architect-v3
...
version: "0.0.1"
category: business
tags: ["seo-structure-architect-v3", "seo-structure-architect", "analyzes", "and", "optimizes", "content", "structure", "including"]
complexity: beginner
risk: safe
...
```text
Use @seo-structure-architect-v3 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @seo-structure-architect-v3 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @seo-structure-architect-v3 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @seo-structure-architect-v3 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-bundle-seo-specialist/skills/seo-structure-architect`, fails to mention provenance, or does not use any copied source files at all.
