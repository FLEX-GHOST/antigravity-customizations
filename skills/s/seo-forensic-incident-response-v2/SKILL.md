---
name: seo-forensic-incident-response-v2
description: "SEO Forensic Incident Response workflow skill. Use this skill when the user needs Investigate sudden drops in organic traffic or rankings and run a structured forensic SEO incident response with triage, root-cause analysis and recovery plan and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: tools
tags: ["seo-forensic-incident-response-v2", "seo-forensic-incident-response", "investigate", "sudden", "drops", "organic", "traffic", "rankings"]
complexity: advanced
risk: caution
...
```text
Use @seo-forensic-incident-response-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @seo-forensic-incident-response-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @seo-forensic-incident-response-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @seo-forensic-incident-response-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/seo-forensic-incident-response`, fails to mention provenance, or does not use any copied source files at all.
