...
tags: ["security-compliance-compliance-check", "you", "are", "compliance", "expert", "specializing", "regulatory", "requirements"]
complexity: beginner
risk: safe
...
```text
Use @security-compliance-compliance-check to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @security-compliance-compliance-check against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @security-compliance-compliance-check for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @security-compliance-compliance-check using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/security-compliance-compliance-check`, fails to mention provenance, or does not use any copied source files at all.
