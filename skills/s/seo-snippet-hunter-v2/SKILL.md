---
name: seo-snippet-hunter-v2
description: "seo-snippet-hunter workflow skill. Use this skill when the user needs Formats content to be eligible for featured snippets and SERP features. Creates snippet-optimized content blocks based on best practices. Use PROACTIVELY for question-based content and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: content-media
tags: ["seo-snippet-hunter-v2", "seo-snippet-hunter", "formats", "content", "eligible", "for", "featured", "snippets"]
complexity: beginner
risk: safe
...
Imported source sections that did not map cleanly to the public headings are still preserved below or in the support files. Notable imported sections: Focus Areas, Snippet Types & Formats, Snippet Optimization Strategy, Approach, Output, [Exact Question from SERP].
...
```text
Use @seo-snippet-hunter-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @seo-snippet-hunter-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @seo-snippet-hunter-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @seo-snippet-hunter-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/seo-snippet-hunter`, fails to mention provenance, or does not use any copied source files at all.
...
#### Imported: Snippet Types & Formats
...
#### Imported: Snippet Optimization Strategy
...
#### Imported: [Exact Question from SERP]
