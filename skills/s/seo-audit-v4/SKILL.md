...
version: "0.0.1"
category: business
tags: ["seo-audit-v4", "seo-audit", "diagnose", "and", "audit", "seo", "issues", "affecting"]
complexity: advanced
risk: caution
...
```text
Use @seo-audit-v4 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @seo-audit-v4 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @seo-audit-v4 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @seo-audit-v4 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-bundle-web-wizard/skills/seo-audit`, fails to mention provenance, or does not use any copied source files at all.
