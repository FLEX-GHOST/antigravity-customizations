---
name: scientific-writing-v2
description: "Scientific Writing workflow skill. Use this skill when the user needs This is the core skill for the deep research and writing tool\u2014combining AI-driven deep research with well-formatted written outputs. Every document produced is backed by comprehensive literature search and verified citations through the research-lookup skill and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: communication
tags: ["scientific-writing-v2", "scientific-writing", "the", "core", "for", "deep", "research", "and"]
complexity: advanced
risk: safe
...
Imported source sections that did not map cleanly to the public headings are still preserved below or in the support files. Notable imported sections: Visual Enhancement with Scientific Schematics, Core Capabilities, Integration with Other Scientific Skills, Assets, Limitations.
...
5. Start with figures and tables (the core data story)
6. For each section below, follow the two-stage process:
7. First: Create outline with bullet points using research-lookup

### Imported Workflow Notes

#### Imported: Workflow for Manuscript Development
...
```text
Use @scientific-writing-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @scientific-writing-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @scientific-writing-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @scientific-writing-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/scientific-writing`, fails to mention provenance, or does not use any copied source files at all.
...
#### Imported: Integration with Other Scientific Skills
