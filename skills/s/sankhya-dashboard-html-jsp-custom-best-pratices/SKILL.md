...
version: "0.0.1"
category: frontend
tags: ["sankhya", "dashboard", "jsp", "html", "sql", "best-practices", "sankhya-dashboard-html-jsp-custom-best-pratices", "should"]
complexity: advanced
risk: caution
...
```text
Use @sankhya-dashboard-html-jsp-custom-best-pratices to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @sankhya-dashboard-html-jsp-custom-best-pratices against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @sankhya-dashboard-html-jsp-custom-best-pratices for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @sankhya-dashboard-html-jsp-custom-best-pratices using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/sankhya-dashboard-html-jsp-custom-best-pratices`, fails to mention provenance, or does not use any copied source files at all.
