...
version: "0.0.1"
category: devops
tags: ["social-media", "cross-channel", "scheduling", "campaigns", "social-orchestrator", "orquestrador", "unificado", "canais"]
complexity: advanced
risk: caution
...
```text
Use @social-orchestrator to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @social-orchestrator against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @social-orchestrator for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @social-orchestrator using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/social-orchestrator`, fails to mention provenance, or does not use any copied source files at all.
