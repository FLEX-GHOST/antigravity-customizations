...
version: "0.0.1"
category: business
tags: ["seo-fundamentals-v3", "seo-fundamentals", "core", "principles", "seo", "including", "e-e-a-t", "web"]
...
| Workflow execution | `scripts/seo_checker.py` | Starts with the smallest copied file that materially changes execution |
| Supporting context | `scripts/seo_checker.py` | Adds the next most relevant copied source file without loading the entire package |
...
```text
Use @seo-fundamentals-v3 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @seo-fundamentals-v3 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @seo-fundamentals-v3 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @seo-fundamentals-v3 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-bundle-seo-specialist/skills/seo-fundamentals`, fails to mention provenance, or does not use any copied source files at all.
...
| `scripts` | upstream helper scripts that change execution or validation | `scripts/seo_checker.py` |
| `agents` | routing or delegation notes that are genuinely part of the imported package | `agents/n/a` |
| `assets` | supporting assets or schemas copied from the source package | `assets/n/a` |

- [seo_checker.py](scripts/seo_checker.py)
- [seo_checker.py](scripts/seo_checker.py)
