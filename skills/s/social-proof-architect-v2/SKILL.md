---
name: social-proof-architect-v2
...
version: "0.0.1"
category: devops
tags: ["social-proof-architect-v2", "social-proof-architect", "one", "sentence", "what", "does", "and", "invoke"]
...
```text
Use @social-proof-architect-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @social-proof-architect-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @social-proof-architect-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @social-proof-architect-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/social-proof-architect`, fails to mention provenance, or does not use any copied source files at all.
