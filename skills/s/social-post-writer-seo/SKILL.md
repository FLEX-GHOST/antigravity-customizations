...
tags: ["social-media", "marketing", "content-writing", "seo", "growth", "social-post-writer-seo", "social", "media"]
...
```text
Use @social-post-writer-seo to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @social-post-writer-seo against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @social-post-writer-seo for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @social-post-writer-seo using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/social-post-writer-seo`, fails to mention provenance, or does not use any copied source files at all.
