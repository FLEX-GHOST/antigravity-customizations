...
version: "0.0.1"
category: design
tags: ["persona", "design-thinking", "product", "presentations", "steve-jobs", "agente", "que", "simula"]
complexity: advanced
risk: caution
...
```text
Use @steve-jobs to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @steve-jobs against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @steve-jobs for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @steve-jobs using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/steve-jobs`, fails to mention provenance, or does not use any copied source files at all.
