...
```text
Use @skill-architect to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Use @skill-architect for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @skill-architect using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
| `scripts` | upstream helper scripts that change execution or validation | `scripts/validate_skill.py` |
...
- [validate_skill.py](scripts/validate_skill.py)
- [examples.md](references/examples.md)
- [patterns.md](references/patterns.md)
