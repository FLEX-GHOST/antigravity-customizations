---
name: seo-content-refresher-v2
description: "seo-content-refresher workflow skill. Use this skill when the user needs Identifies outdated elements in provided content and suggests updates to maintain freshness. Finds statistics, dates, and examples that need updating. Use PROACTIVELY for older content and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: content-media
tags: ["seo-content-refresher-v2", "seo-content-refresher", "identifies", "outdated", "elements", "provided", "content", "and"]
complexity: beginner
risk: safe
...
```text
Use @seo-content-refresher-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @seo-content-refresher-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @seo-content-refresher-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @seo-content-refresher-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
#### Imported: Content Freshness Guidelines
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/seo-content-refresher`, fails to mention provenance, or does not use any copied source files at all.
...
#### Imported: Refresh Priority Matrix
