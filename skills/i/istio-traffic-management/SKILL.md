...
description: "Istio Traffic Management workflow skill. Use this skill when the user needs Comprehensive guide to Istio traffic management for production service mesh deployments and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: tools
tags: ["istio-traffic-management", "comprehensive", "guide", "istio", "traffic", "management", "for", "production"]
...
```text
Use @istio-traffic-management to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @istio-traffic-management against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @istio-traffic-management for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @istio-traffic-management using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/istio-traffic-management`, fails to mention provenance, or does not use any copied source files at all.
