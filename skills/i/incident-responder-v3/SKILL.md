...
description: "incident-responder workflow skill. Use this skill when the user needs Expert SRE incident responder specializing in rapid problem resolution, modern observability, and comprehensive incident management and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: devops
tags: ["incident-responder-v3", "incident-responder", "expert", "sre", "incident", "responder", "specializing", "rapid"]
complexity: advanced
risk: caution
...
```text
Use @incident-responder-v3 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @incident-responder-v3 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @incident-responder-v3 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @incident-responder-v3 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-bundle-observability-monitoring/skills/incident-responder`, fails to mention provenance, or does not use any copied source files at all.
