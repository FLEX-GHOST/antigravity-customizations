---
name: itil-expert-v2
description: "ITIL Expert (ITIL 4 & 5) workflow skill. Use this skill when the user needs Expert advisor for ITIL 4 and ITIL 5 (2026 digital product paradigm), specialized in AI-native governance, sustainability, and value co-creation and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: devops
tags: ["itil-expert-v2", "itil-expert", "expert", "advisor", "for", "itil", "and", "digital"]
...
```text
Use @itil-expert-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @itil-expert-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @itil-expert-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @itil-expert-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/itil-expert`, fails to mention provenance, or does not use any copied source files at all.
...
- [itil-5-evolution.md](references/itil-5-evolution.md)
- [itil-usage.md](examples/itil-usage.md)
- [README.md](README.md)
- [itil-usage.md](examples/itil-usage.md)
- [itil-5-evolution.md](references/itil-5-evolution.md)
