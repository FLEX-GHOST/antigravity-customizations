...
version: "0.0.1"
category: tools
tags: ["idea-darwin-v2", "idea-darwin", "darwinian", "idea", "evolution", "engine", "toss", "rough"]
...
```text
Use @idea-darwin-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/idea-darwin`, fails to mention provenance, or does not use any copied source files at all.
