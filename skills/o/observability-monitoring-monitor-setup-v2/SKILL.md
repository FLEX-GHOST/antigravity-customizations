---
name: observability-monitoring-monitor-setup-v2
description: "Monitoring and Observability Setup workflow skill. Use this skill when the user needs You are a monitoring and observability expert specializing in implementing comprehensive monitoring solutions. Set up metrics collection, distributed tracing, log aggregation, and create insightful da and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: devops
tags: ["observability-monitoring-monitor-setup-v2", "observability-monitoring-monitor-setup", "you", "are", "monitoring", "and", "observability", "expert"]
complexity: beginner
risk: safe
...
- Use when the request clearly matches the imported source intent: You are a monitoring and observability expert specializing in implementing comprehensive monitoring solutions. Set up metrics collection, distributed tracing, log aggregation, and create insightful da.
...
```text
Use @observability-monitoring-monitor-setup-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @observability-monitoring-monitor-setup-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @observability-monitoring-monitor-setup-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @observability-monitoring-monitor-setup-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/observability-monitoring-monitor-setup`, fails to mention provenance, or does not use any copied source files at all.
