...
version: "0.0.1"
category: business
tags: ["odoo-sales-crm-expert-v3", "odoo-sales-crm-expert", "expert", "guide", "for", "odoo", "sales", "and"]
...
- Use when the request clearly matches the imported source intent: Expert guide for Odoo Sales and CRM: pipeline stages, quotation templates, pricelists, sales teams, lead scoring, and forecasting.
...
```text
Use @odoo-sales-crm-expert-v3 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @odoo-sales-crm-expert-v3 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @odoo-sales-crm-expert-v3 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @odoo-sales-crm-expert-v3 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-bundle-odoo-erp/skills/odoo-sales-crm-expert`, fails to mention provenance, or does not use any copied source files at all.
