...
version: "0.0.1"
category: devops
tags: ["observability-monitoring-monitor-setup", "you", "are", "monitoring", "and", "observability", "expert", "specializing"]
complexity: beginner
risk: safe
...
```text
Use @observability-monitoring-monitor-setup to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @observability-monitoring-monitor-setup against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @observability-monitoring-monitor-setup for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @observability-monitoring-monitor-setup using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/observability-monitoring-monitor-setup`, fails to mention provenance, or does not use any copied source files at all.
