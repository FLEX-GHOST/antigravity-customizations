---
name: odoo-security-rules-v3
description: "Odoo Security Rules workflow skill. Use this skill when the user needs Expert in Odoo access control: ir.model.access.csv, record rules (ir.rule), groups, and multi-company security patterns and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: testing-security
tags: ["odoo-security-rules-v3", "odoo-security-rules", "expert", "odoo", "access", "control", "model", "csv"]
...
```text
Use @odoo-security-rules-v3 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @odoo-security-rules-v3 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @odoo-security-rules-v3 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @odoo-security-rules-v3 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-bundle-odoo-erp/skills/odoo-security-rules`, fails to mention provenance, or does not use any copied source files at all.
