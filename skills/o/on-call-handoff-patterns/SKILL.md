...
description: "On-Call Handoff Patterns workflow skill. Use this skill when the user needs Effective patterns for on-call shift transitions, ensuring continuity, context transfer, and reliable incident response across shifts and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: devops
tags: ["on-call-handoff-patterns", "effective", "patterns", "for", "on-call", "shift", "transitions", "ensuring"]
complexity: advanced
risk: caution
...
```text
Use @on-call-handoff-patterns to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @on-call-handoff-patterns against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @on-call-handoff-patterns for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @on-call-handoff-patterns using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/on-call-handoff-patterns`, fails to mention provenance, or does not use any copied source files at all.
