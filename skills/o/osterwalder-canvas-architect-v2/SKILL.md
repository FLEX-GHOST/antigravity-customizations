---
name: osterwalder-canvas-architect-v2
description: "Osterwalder Business Model Canvas Architect workflow skill. Use this skill when the user needs Iterative consultant agent for building and validating logically consistent 9-block Business Model Canvases and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: frontend
tags: ["business-model", "osterwalder", "strategy", "bmc", "osterwalder-canvas-architect-v2", "osterwalder-canvas-architect", "iterative", "consultant"]
complexity: beginner
risk: safe
...
- Use when the request clearly matches the imported source intent: Iterative consultant agent for building and validating logically consistent 9-block Business Model Canvases.
...
```text
Use @osterwalder-canvas-architect-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @osterwalder-canvas-architect-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @osterwalder-canvas-architect-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @osterwalder-canvas-architect-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/osterwalder-canvas-architect`, fails to mention provenance, or does not use any copied source files at all.
