...
version: "0.0.1"
category: business
tags: ["launch-strategy-v3", "launch-strategy", "you", "are", "expert", "saas", "product", "launches"]
complexity: advanced
risk: caution
...
```text
Use @launch-strategy-v3 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @launch-strategy-v3 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @launch-strategy-v3 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @launch-strategy-v3 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-bundle-startup-founder/skills/launch-strategy`, fails to mention provenance, or does not use any copied source files at all.
