---
name: leiloeiro-juridico-v2
description: "SKILL JUR\u00cdDICA \u2014 LEIL\u00d5ES DE IM\u00d3VEIS workflow skill. Use this skill when the user needs Analise juridica de leiloes: nulidades, bem de familia, alienacao fiduciaria, CPC arts 829-903, Lei 9514/97, onus reais, embargos e jurisprudencia and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: devops
tags: ["legal", "auction-law", "brazilian", "judicial", "leiloeiro-juridico-v2", "leiloeiro-juridico", "analise", "juridica"]
complexity: advanced
risk: caution
...
Imported source sections that did not map cleanly to the public headings are still preserved below or in the support files. Notable imported sections: How It Works, Para O Arrematante/Investidor:, Para O Devedor/Executado:, 5. Glossário Jurídico Essencial, 6. Fraude À Execução (Art. 792 Cpc), 7. Regularização Fundiária (Lei 13.465/2017 — Reurb).
...
```text
Use @leiloeiro-juridico-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @leiloeiro-juridico-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @leiloeiro-juridico-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @leiloeiro-juridico-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/leiloeiro-juridico`, fails to mention provenance, or does not use any copied source files at all.
