---
name: llm-structured-output-v2
description: "LLM Structured Output workflow skill. Use this skill when the user needs > and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: ai-agents
tags: ["llm-structured-output-v2", "llm-structured-output", "ai-agents"]
complexity: advanced
risk: caution
...
Imported source sections that did not map cleanly to the public headings are still preserved below or in the support files. Notable imported sections: What This Skill Does, Never Do This, Edge Cases, Limitations.
...
- The user asks about responseformat, jsonmode, jsonobject, or jsonschema in OpenAI
- The user asks about using Anthropic's tooluse or toolresult blocks for data extraction (not for actual tool execution)
...
3. OpenAI (gpt-4o, gpt-4o-mini): Use responseformat: { type: "jsonschema", json_schema: { ... } }. This enables Structured Outputs with guaranteed schema conformance via constrained decoding.
4. Anthropic (Claude): Define a single tool with the target schema as inputschema and set toolchoice: { type: "tool", name: "extractdata" }. Claude returns the structured data in the tooluse content block.
5. Google (Gemini): Use generationConfig.responseSchema with a JSON Schema object and set responseMimeType: "application/json".
6. Local models (llama.cpp, vLLM): Use GBNF grammars or --json-schema flag for constrained decoding at the token level.
7. Write the schema definition in the user's language. For Python, define a Pydantic BaseModel. For TypeScript, define a Zod schema and convert it with zodResponseFormat(). For raw API calls, write JSON Schema directly.
...
```text
Use @llm-structured-output-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @llm-structured-output-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @llm-structured-output-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @llm-structured-output-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/llm-structured-output`, fails to mention provenance, or does not use any copied source files at all.
