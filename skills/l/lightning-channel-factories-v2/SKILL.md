---
name: lightning-channel-factories-v2
description: "lightning-channel-factories workflow skill. Use this skill when the user needs Technical reference on Lightning Network channel factories, multi-party channels, LSP architectures, and Bitcoin Layer 2 scaling without soft forks. Covers Decker-Wattenhofer, timeout trees, MuSig2 key aggregation, HTLC/PTLC forwarding, and watchtower breach detection and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: development
tags: ["lightning-channel-factories-v2", "lightning-channel-factories", "technical", "reference", "lightning", "network", "channel", "factories"]
complexity: beginner
risk: caution
...
```text
Use @lightning-channel-factories-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @lightning-channel-factories-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @lightning-channel-factories-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @lightning-channel-factories-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/lightning-channel-factories`, fails to mention provenance, or does not use any copied source files at all.
