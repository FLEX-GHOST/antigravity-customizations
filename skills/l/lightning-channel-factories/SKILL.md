...
tags: ["lightning-channel-factories", "technical", "reference", "lightning", "network", "channel", "factories", "multi-party"]
complexity: beginner
risk: caution
...
```text
Use @lightning-channel-factories to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @lightning-channel-factories against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @lightning-channel-factories for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @lightning-channel-factories using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/lightning-channel-factories`, fails to mention provenance, or does not use any copied source files at all.
