---
name: loki-mode
description: "Loki Mode - Multi-Agent Autonomous Startup System workflow skill. Use this skill when the user needs Version 2.35.0 | PRD to Production | Zero Human Intervention > Research-enhanced: OpenAI SDK, DeepMind, Anthropic, AWS Bedrock, Agent SDK, HN Production (2025) and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: devops
tags: ["loki-mode", "version", "prd", "production", "zero", "human", "intervention", "research-enhanced"]
complexity: advanced
risk: caution
...
# Loki Mode - Multi-Agent Autonomous Startup System > Version 2.35.0 | PRD to Production | Zero Human Intervention > Research-enhanced: OpenAI SDK, DeepMind, Anthropic, AWS Bedrock, Agent SDK, HN Production (2025) ---

Imported source sections that did not map cleanly to the public headings are still preserved below or in the support files. Notable imported sections: Prerequisites, RARV Cycle (Every Iteration), Model Selection Strategy, Tool Orchestration & Efficiency, Structured Prompting for Subagents, GOAL (What success looks like).

## When to Use This Skill

Use this section as the trigger filter. It should make the activation boundary explicit before the operator loads files, runs commands, or opens a pull request.

- Report back with: WHY, WHAT, TRADE-OFFS, RISKS ``` --- This skill is applicable to execute the workflow or actions described in the overview.
- Use when the request clearly matches the imported source intent: Version 2.35.0 | PRD to Production | Zero Human Intervention > Research-enhanced: OpenAI SDK, DeepMind, Anthropic, AWS Bedrock, Agent SDK, HN Production (2025).
...
| Workflow execution | `references/advanced-patterns.md` | Starts with the smallest copied file that materially changes execution |
| Supporting context | `references/agent-types.md` | Adds the next most relevant copied source file without loading the entire package |
...
```text
Use @loki-mode to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @loki-mode against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @loki-mode for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @loki-mode using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
- NEVER edit autonomy/run.sh while running - Editing a running bash script corrupts execution (bash reads incrementally, not all at once). If you need to fix run.sh, note it in CONTINUITY.md for the next session.
...
#### Imported: Constitutional AI Principles (Anthropic)
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills-claude/skills/loki-mode`, fails to mention provenance, or does not use any copied source files at all.
...
| --- | --- | --- |
| `references` | copied reference notes, guides, or background material from upstream | `references/advanced-patterns.md` |
| `examples` | worked examples or reusable prompts copied from upstream | `examples/api-only.md` |
| `scripts` | upstream helper scripts that change execution or validation | `scripts/export-to-vibe-kanban.sh` |
...
- [business-ops.md](references/business-ops.md)
- [api-only.md](examples/api-only.md)
- [full-stack-demo.md](examples/full-stack-demo.md)
- [simple-todo-app.md](examples/simple-todo-app.md)
- [static-landing-page.md](examples/static-landing-page.md)
...
#### Imported: RARV Cycle (Every Iteration)
...
#### Imported: Model Selection Strategy
...
#### Imported: Tool Orchestration & Efficiency
...
#### Imported: Structured Prompting for Subagents
...
#### Imported: GOAL (What success looks like)
...
#### Imported: Red Flags - Never Do These
...
#### Imported: Multi-Tiered Fallback System
...
#### Imported: AGENTS.md Integration
...
#### Imported: Debate-Based Verification (DeepMind)
...
#### Imported: Production Patterns (HN 2025)
...
#### Imported: Directory Structure Overview
