...
# n8n MCP Tools Expert Master guide for using n8n-mcp MCP server tools to build workflows.

Imported source sections that did not map cleanly to the public headings are still preserved below or in the support files. Notable imported sections: Tool Categories, Tool Selection Guide, Critical: nodeType Formats, Detailed Guides, Self-Help Tools, Tool Availability.
...
- Use when the request clearly matches the imported source intent: Expert guide for using n8n-mcp MCP tools effectively. Use when searching for nodes, validating configurations, accessing templates, managing workflows, or using any n8n-mcp tool. Provides tool selection guidance,....
...
```text
Use @n8n-mcp-tools-expert-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @n8n-mcp-tools-expert-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @n8n-mcp-tools-expert-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @n8n-mcp-tools-expert-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/n8n-mcp-tools-expert`, fails to mention provenance, or does not use any copied source files at all.
...
#### Imported: Unified Tool Reference
...
#### Imported: Critical: nodeType Formats
...
#### Imported: Performance Characteristics
