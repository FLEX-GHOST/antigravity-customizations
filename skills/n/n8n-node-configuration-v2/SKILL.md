---
name: n8n-node-configuration-v2
description: "n8n Node Configuration workflow skill. Use this skill when the user needs Operation-aware node configuration guidance. Use when configuring nodes, understanding property dependencies, determining required fields, choosing between get_node detail levels, or learning common configuration patterns by node type and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: devops
tags: ["n8n-node-configuration-v2", "n8n-node-configuration", "operation-aware", "node", "configuration", "guidance", "use", "configuring"]
complexity: advanced
risk: caution
...
5. If field unclear → getnode({mode: "searchproperties"})
...
```text
Use @n8n-node-configuration-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @n8n-node-configuration-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @n8n-node-configuration-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @n8n-node-configuration-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/n8n-node-configuration`, fails to mention provenance, or does not use any copied source files at all.
