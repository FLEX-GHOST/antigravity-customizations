---
name: notion-template-business-v2
description: "Notion Template Business workflow skill. Use this skill when the user needs Expert in building and selling Notion templates as a business - not and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: business
tags: ["notion-template-business-v2", "notion-template-business", "expert", "building", "and", "selling", "notion", "templates"]
complexity: advanced
risk: safe
...
```text
Use @notion-template-business-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @notion-template-business-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @notion-template-business-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @notion-template-business-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/notion-template-business`, fails to mention provenance, or does not use any copied source files at all.
