...
version: "0.0.1"
category: backend
tags: ["mcp", "search", "ai-discovery", "api-discovery", "mcp-verification", "agent-tools", "not-human-search-mcp-v2", "not-human-search-mcp"]
...
```text
Use @not-human-search-mcp-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @not-human-search-mcp-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @not-human-search-mcp-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @not-human-search-mcp-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/not-human-search-mcp`, fails to mention provenance, or does not use any copied source files at all.
