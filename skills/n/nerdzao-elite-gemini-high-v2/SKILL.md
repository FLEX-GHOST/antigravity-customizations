---
name: nerdzao-elite-gemini-high-v2
description: "@nerdzao-elite-gemini-high workflow skill. Use this skill when the user needs Modo Elite Coder + UX Pixel-Perfect otimizado especificamente para Gemini 3.1 Pro High. Workflow completo com foco em qualidade m\u00e1xima e efici\u00eancia de tokens and the operator should preserve the upstream workflow, copied support files, and provenance before merging or handing off."
version: "0.0.1"
category: development
tags: ["nerdzao-elite-gemini-high-v2", "nerdzao-elite-gemini-high", "modo", "elite", "coder", "pixel-perfect", "otimizado", "especificamente"]
complexity: beginner
risk: caution
...
- Use when the request clearly matches the imported source intent: Modo Elite Coder + UX Pixel-Perfect otimizado especificamente para Gemini 3.1 Pro High. Workflow completo com foco em qualidade máxima e eficiência de tokens.
...
```text
Use @nerdzao-elite-gemini-high-v2 to handle <task>. Start from the copied upstream workflow, load only the files that change the outcome, and keep provenance visible in the answer.
```
...
```text
Review @nerdzao-elite-gemini-high-v2 against metadata.json and ORIGIN.md, then explain which copied upstream files you would load first and why.
```
...
```text
Use @nerdzao-elite-gemini-high-v2 for <task>. Load only the copied references, examples, or scripts that change the outcome, and name the files explicitly before proceeding.
```
...
```text
Review @nerdzao-elite-gemini-high-v2 using the copied upstream files plus provenance, then summarize any gaps before merge.
```
...
**Symptoms:** The result ignores the upstream workflow in `plugins/antigravity-awesome-skills/skills/nerdzao-elite-gemini-high`, fails to mention provenance, or does not use any copied source files at all.
